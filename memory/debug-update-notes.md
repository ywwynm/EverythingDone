## 光带右下起步、内容放大承接与涟漪圆心（20260914122202）

## 修复涟漪档保存后卡片级揭示在长滚动后直接出现（20260914145837）

发布后核验：debug 更新 `202609140658`，26752131 字节，SHA-256 `3032739e38aabb51c59a2d4396fd756a656cf66c7bf74b3e1f658977840e2b4d`，latest.json 含 688 字 releaseNotes。两台测试机装的是发布前同源构建（`e7dcb3a3…`），差异只在构建时间戳。

1. 用户反馈涟漪档新建完成后卡片级涟漪缺失，场景为置顶记事很多、需先滚动到新记事。R5CW20BLNKL 以 20 张高卡片（约 4.5 屏）置顶块复现：揭示动画启动后 11 ms 即结束，卡片整张弹出；根因是卡片 View 在脱离窗口时被绑定、重新挂回后从未绘制，RenderThread 动画在其 RenderNode 上立即完成。
2. 修复：揭示前等待期让卡片 alpha 0 可见并先结束 ItemAnimator 插入淡入；卡片级边框光效同样处理；平滑滚动兜底改为轮询等待滚动结束（硬上限 9 s），IDLE 时只要仍在门控就重试。粒子档未改。
3. 修复后三星 4.5 屏场景涟漪 550 ms 完整、边框卡片淡入正常、带小段滚动新建无提前露出；OPPO 原地涟漪 5 倍时长下 2720 ms 完整。7 项相关 JVM 测试通过。
4. 两台设备 39 条测试记事经真实多选工具栏移入可恢复的回收站（三星回到原 3 条，OPPO 回到 2 文件夹 15 条），动画档位恢复（OPPO 粒子、三星边框），动画时长缩放恢复。未创建 Git 提交。证据在 `tmp/thing-animation-device/ripple-card-2026-09-14/`。

发布后核验：debug 更新 `202609140422`，远端 latest.json 含完整 releaseNotes，本地发布产物与两台设备实际安装包 SHA-256 均为 `6ab1367e461e6a74b895a2af921bcbc4d7472fb6343677c699360fdb471d2bb1`。实际发布包在 Samsung 再跑一轮全屏光带（1276.8 ms）与一轮保存卡片光带（进程内第一次播放即落在卡片上），在 OPPO 再跑一轮全屏光带；两台原动画档位（OPPO 粒子、Samsung 边框光效）、动画时长缩放与自动旋转均已恢复，专用测试记事全部经真实界面移入可恢复的回收站，最终无 ParticleDismissGl 线程、无本轮进程崩溃。未创建 Git 提交。

1. 全屏光带改为从右下角起步：底边向左 → 左边向上 → 顶边向右 → 右边向下，回到右下角。`ShiningBorder` 新增起点角选项与一条右下起点的顺时针路径，不再依赖此前未生效的 `scaleX = -1` 水平翻转；1290 ms 时长、颜色、8dp 描边与粒子参数不变。
2. 光带结束后编辑页内容以右下角为轴从 0.84 放大到 1，216 ms 减速插值，全程完全不透明（按用户第二次裁定去掉了淡入）。放大期间首页快照仍在底层，结束才撤快照并弹键盘；所有中断路径复位缩放与透明度。
3. 保存后的卡片级光带同样改为卡片右下角起步，1600 ms、末尾 220 ms 淡入与中断保护不变。顺带修掉一个既有缺陷：显式指定矩形后仍会被 `onDraw` / `startAnimation` 用 View 自身边界覆盖成整屏，导致进程内第一次保存看到整屏光带。
4. 涟漪档圆心由内容右下角改回新建按钮中心，终止半径取到最远角，600 ms 不变；粒子档未改。
5. 两台真机逐项验收：Samsung 逐帧时间戳给出光带 1267–1285 ms、放大 207–210 ms、合计 1486–1502 ms，放大各帧与最终页面同缩放逐像素一致（差 ≤0.96/255），涟漪圆心与 FAB 中心差 0.0–1.0 px；OPPO 因 screenrecord 被拒改用截图连拍（约 2 fps）并配合动画时长缩放取几何证据，圆心差 (−1.0, −0.2) px。
6. 35 项相关 JVM 测试通过，其中 5 项为新增的边框路径几何测试（右下起点与左下起点各覆盖直角与圆角）。
...
发布后核验：调试更新 `202609131811` 已发布并安装到 9018f404，远端完整下载、本地及安装包 SHA-256 均为 `5b08d9a748d6152ff1a936ab7d5ab05d587497ca41ccbd390fb8087cc2a57843`，共 26276578 字节。发布包真实新建首帧准备为 333 ms；两轮左滑首个非零进度约在识别后 43 ms。保存三阶段顺序与图片圆角再次通过。四条专用记事通过实际菜单移入可恢复的回收站，原 15 条记事、2 个文件夹、动画设置与自动旋转恢复，采样关闭，最终粒子和观察器线程均为 0，无当前进程崩溃。未创建 Git 提交。
...
发布后核对：调试更新 `202609131502` 已发布并安装到 9018f404；远端完整下载、本地与设备安装包 SHA-256 均为 `8f38efe7695f6d926c81623587008a8d70a178de5a11e3aa7abe8c38bcbb846c`。发布包另外三轮提前返回及正常新建通过。六条测试记事移入可恢复的回收站，原首页、涟漪／滑动设置和自动旋转已恢复。未创建 Git 提交。
