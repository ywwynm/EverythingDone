...
发布日志：docs/features/dialog-particle-dismiss/debug-updates/update-20260913211407.md。
...
发布身份与复核：阿里云 `202609131102`，完整远端、本地及 9018f404 已安装 APK 的 SHA-256 为 `5d02cbeb9f4b1b32c2db5e0d3d17898fd054a84ca0c28647aa231ab104aed43e`，25,801,070 字节。六轮真实工具栏出现 35–37 帧；三轮附件→录音 12 个动画通过帧预算，Surface 交接像素、快速关闭、两轮 12 张海浪首帧捕获通过。普通附件关闭仍有两次约 68／82 ms 起步空档，严格联合检查失败保留。三星未连接。发布日志：docs/features/dialog-particle-dismiss/debug-updates/update-20260913190300.md。
...
本轮发布日志：docs/features/dialog-particle-dismiss/debug-updates/update-20260913173241.md。发布及安装身份复核结果随后记录在功能文档中。
...
用户指出 ripple 截断是共同弹窗问题，添加附件切录音因下一窗口复杂而最明显。已用真实 DetailActivity、AddAttachmentDialogFragment、AudioRecordDialogFragment 触摸重放复现；源窗口撤下时反馈仍有 45%～78% 透明度，源消散最长 Texture 提交间隔达 166 ms。

ParticleTouchFeedback 与 ParticleDismissController 在真实反馈继续播放期间准备干净快照，待实际淡出后交给粒子；普通返回和背景关闭不增加等待。BackgroundUtil 与 GradientRippleDrawable 统一弹窗中性、强调色、圆形反馈，移除重复前景；录音、音频播放、海浪参数、开始做事和文件夹选择沿用共同入口。业务回调仍在点击时执行。

ParticleGpuWork 按小批次限制逆向准备队列；ParticleWindowFrameRate 与 TextureView 在动画期间表达刷新率需求并恢复原策略。FableSol 正常 GLES 不再创建完整 Canvas 备用模拟，真正回退时恢复状态；源海浪临时冻结延续至消散结束。没有改变共同粒子数量、模型、资源、方向和时长。

9018f404 七条真实路径通过；最终该轮录音交接源消散最长间隔 19 ms、出现 18 ms、录音关闭 27 ms。复杂界面首次创建仍有约 60～80 ms 主线程间隔，系统也可能按约 30 Hz 合成，不声明所有情况零等待或稳定 60 fps。31 项 JVM／GLSL／冻结检查、111 个 GPU 正反对应帧、三组干净快照整图比较、三组出现配色尺寸、五组海浪与生命周期回归通过。三星未连接，未开始实际录音或修改记事。

发布日志：docs/features/dialog-particle-dismiss/debug-updates/update-20260913145709.md。阿里云 `202609130700` 发布复核完成：完整 APK 为 29,022,586 字节，SHA-256 `5afc7221d10dcf0b2f0082e8104b68c0f5e21846e804331761ffd906ec54e69b`，远端、本地与 9018f404 安装包一致。发布包录音交接、灰色取消、返回三组通过；录音出现最长帧间隔 17 ms、消失 34 ms，首次创建仍有 85 ms 主线程间隔。三星仍未连接。本轮未提交 Git。

## 补齐弹窗、调整外观面板和音频海浪的粒子动画

用户指出开始做事、更显著的通知、首页调整外观，以及录音、音频播放和 FableSol 参数弹窗尚未接入，要求检查其它遗漏。本版统一使用已认可的粒子模型，保留下方随机进入、0.6 秒出现及一秒消失。

修复透明宿主尚未附着就跳过出现、Surface／Texture 整体排除、Activity 提前退出三类原因。ParticleDismissController 将异步捕获准备也纳入宿主退出等待；ParticleDialogHostActivity、ActivityContentDialogFragment 和 ParticlePanelAnimator 分别承接透明宿主、浮动通知／小组件与首页内嵌面板。恢复浮动内容原宽度，去除宿主与弹窗叠加的背景变暗。

ParticleSnapshot 将独立海浪缓冲合入普通窗口文字下方，FableSol 在捕获与交接期间临时冻结，恢复时保留原暂停／导出状态。隐藏 Surface 不参与，Texture 不再阻断；普通弹窗继续原捕获方式。共同模型与桌面视频没有改动。

9018f404 已从实际入口检查开始做事、调整外观、录音、参数、通知、小组件；真实音频播放窗口使用固定静音文件。独立 Surface、Texture、浮动页面、同步退出及快速关闭通过真实 GPU 检查。22 项 JVM／GLSL 通过，选择／更新日志／颜色信息整图像素、尺寸和位置相等。三星未连接，实际相机与 Canvas 回退未做设备验证；冷 Surface 准备等待仍可能可见。

本次发布日志使用 docs/features/dialog-particle-dismiss/debug-updates/update-20260913133725.md。发布后核对完整远端包、安装身份与关键入口；尚未提交 Git。

发布复核完成：阿里云 `202609130540`，完整 APK 为 26,547,272 字节，SHA-256 `9a538fbd4ff6e213e2709163b33c0ba25769860ba8ba66c0abd26769fb36bd81`。远端、本地及 9018f404 安装包一致；发布包四组关键窗口完成两向动画和清理，更新日志整图、尺寸与位置回归一致。三星仍未连接。
...
用户希望先比较几种出现动效选项，并报告标题、确定按钮等在出现动画结束瞬间变色。新动效先提供四种设计示意；本包修复已有出现动画的配色交接。

实际取出真实 Chooser / Alert 弹窗的动画 Bitmap，和最终 View 逐像素比较：原 Chooser 标题、确认按钮及选中项文字的 14,956 个像素存在明显变化。原因是 `ParticleDismissController.startCondense` 抓图先于同一轮 pre-draw 中的文字渐变初始化。现把捕获移到本次绘制准备完成后，无固定毫秒延迟，并检查窗口显示与附着状态。共同消散模型继续沿用。

9018f404 上两类弹窗 × 正向渐变／反向渐变／纯色共六组整图最大通道差全部为 0；四种真实关闭方式及两次快速返回通过，四条关闭路径首帧为 90–108 毫秒。R5CW20BLNKL 当前离线待补。本次发布日志传入 `docs/features/dialog-particle-dismiss/debug-updates/update-20260913004746.md`，发布后验证完整远端包、安装身份和实际界面行为。

发布结果：阿里云更新 `202609121648` 已安装 9018f404，完整远端包、本地发布包与安装包 SHA-256 一致。最终包六组真实弹窗整图像素差全部为 0，四条关闭路径首帧平均约 100 毫秒、首次合成约 109 毫秒。R5CW20BLNKL 仍离线待补。

## 弹窗粒子动画：缩短关闭操作后的启动等待

用户认可的视觉阶段已先提交为 `d37b42cb`。随后针对背景触摸、返回、取消与确认，定位约四百毫秒的关闭后建材开销，增加本地批量数值计算、稳定排序及缓冲打包；GPU 公共准备与后台建材重叠，并预热首次驱动编译。共同视觉规则与粒子数量不变，保留 Kotlin 独立参考及回退。

9018f404 四条真实关闭路径各两次，首帧等待从平均约 463 毫秒降为约 99 毫秒，界面首次合成平均约 107 毫秒。40 组材料及上传缓冲逐值一致；三个素材与 Kotlin 参考路径、两个素材与认可版本输出的 45 次关键帧比较全部为零差值。19 项 JVM 检查通过，四种 ABI 的新增本地库加载段为 16 KB 对齐。发布后校验远端和已安装包，并复测实际关闭；R5CW20BLNKL 当前离线待补。

本次发布任务传入 `docs/features/dialog-particle-dismiss/debug-updates/update-20260912231220.md`，诊断位于该功能的 `startup-latency-2026-09-12.md`。

发布结果：阿里云更新 `202609121513` 已安装至 9018f404，远端完整 APK、本地发布包与已安装包 SHA-256 一致，共同模型资源逐字节一致。最终包四条关闭路径首帧等待平均约 106 毫秒、首次界面合成平均约 111 毫秒，关闭及动画层清理通过。R5CW20BLNKL 仍离线待补。

## 弹窗消散：约束当前位置的剥离汇聚，保留正常边缘卷动

用户否定随机打散与静态压缩收减，并要求孤立细缕彻底不再出现、钢铁侠直接对比华为原片。本轮按粒子当前位置累计质量和剥离动量，添加单侧压力修正；保留原速度与边缘卷动，不靠透明度或寿命隐藏细线。桌面和 Android 使用同一计算，修复桌面重复播放状态清理。

三个确定反例及其他材质／方向／种子的 12 组完整过程已复核；18 素材、30 方向、14 项 JVM 和 150 个视频检查通过。9018f404 九组 GPU 对照与实际近远／返回关闭通过，R5CW20BLNKL 当前离线待补。钢铁侠对原片的结构误差略降，尚不宣称像素级一致。

发布任务传入 `docs/features/dialog-particle-dismiss/debug-updates/update-20260912210037.md`，完整诊断记录位于该功能目录的 `edge-flow-support-2026-09-12.md`。

发布结果：阿里云更新 `202609121309` 已安装到 9018f404；远端、本地和已安装 APK 散列一致，最终包两组独立 GPU 对照、实际近远／返回关闭通过。一次验证图片传输失败已恢复并重新核对。R5CW20BLNKL 仍未连接，待补测。

## 弹窗消散：恢复弧边分支与可见的近远移动差异

用户指出钢铁侠横向弧边与下方拖尾的分离仍未复现，近、中、远也仍不明显。逐项对照定位到随机坐标及释放变换造成源材料错位；共同规则现在按局部起点独立程度调节变形幅度，保留镜像与其他随机布局。实际触点按该方向可用屏幕背景归一化，温和改变速度和位移，不作强点汇聚。

桌面与 Android 的随机、触点换算和共享着色器同步；共同场未更换。144 视频包含新局部三栏对照，234 状态、18 场景、30 方向、13 JVM 测试及 9018f404 的 13 组独立渲染与真实关闭通过。三星因 USB 调试未授权待补；参考亮度、释放时序与细缕密度仍有差异。

本次完整发布日志位于 `docs/features/dialog-particle-dismiss/debug-updates/update-20260911134000.md`，发布任务传入该文件。发布后核对 APK、共享资源和远端散列。

发布结果：调试更新 `202609110541` 已发布并安装到 9018f404，远端完整 APK 与本地／安装包散列一致。发布后返回关闭与三次附件弹窗随机关闭通过；三星仍待 USB 调试授权。
...
发布结果：阿里云更新 202609101930 已发布；两台指定设备安装相同发布包，返回关闭和动画层清理通过。远端 APK 字节与九份共享资源已校验，SHA-256：a5e6bae6d6e04ff99eb870f6a39df8f964536fc2133765221d11a452dc00ea01。代码未提交。
...
发布号 `202608310340`，versionCode 43 / versionName 2.0.0，
APK SHA-256 `fbbcfc6f281e5b7cbf786f739ea6f4826dae4866d10918ea4bf66712bcb8e07b`，
24977248 字节。日志文件
`docs/features/dialog-particle-dismiss/debug-updates/update-20260831113945.md`，
已核对 latest.json 含完整 releaseNotes（1086 字符）且 sha256 与本地 APK 一致。

对话框消散动画的四类观感问题逐条改进：静止层在释放前按参考实测的曲线压暗到 0.78、
新生粒子从 0.86 起步；顺风角取消分形抑制、改为错开释放并按错开系数重新定标提前量；
折叠位移在轮廓处收敛到零，右／下越界从 9.4% 归零；网格 512 → 256 列并新增逐粒子粒径
倍率。另纠正了一处运动模型错误——正比于离轴距离的横向发散项已删除，膨胀／平移之比
从 0.24 降到 0.10。真机 R5CW20BLNKL：124 帧、卡顿 1.61%、90 分位 14 ms。

## 2026-08-30 - 开场顺风自由边持续粒子化与单一弯曲帷幔

用户指出左上消逝时，左上角、上边缘和左边缘只象征性起粒，随后近似停住，Dialog 留存
区域几乎不变，左上牵伸也不足。诊断确认旧实现只提前少量表层粒子的出生时钟，完整层仍
等待主体释放，且早发纤维的有效飞行年龄在短窗口后近似封顶。

本轮在 `ParticleDismissReleaseField.kt` 中把顺风角和相邻两条自由边通过有限边界层的平滑
并集纳入同一个连续到达场；`ParticleDismissRenderer.kt` 的完整层与粒子层读取相同相位，
早发纤维在 `t>0.20` 后仍保持正输运斜率。唯一剥离前沿增加切向单峰弓形，细孔只在同一
相位的高张力过渡区产生；帷幔仍用更多同尺寸真实粒子增密。彩色内容与中性色共用全部
动力学，直接粒子数量表示保持 `4.49×`。

所有形态、激活、运动、连贯性、边缘、方向、跨分辨率、颜色和真实 GLSL 回归通过，
`:app:testDebugUnitTest :app:assembleDebug` 成功。在授权设备 `R5CW20BLNKL` 上以真实左上
外部触点关闭“添加附件”：`t=0.10` 已出现左上缺口与外移粒子，`t=0.16` 左边缘继续扩展，
`t=0.216–0.32` 上边缘持续推进。最终发布 APK 的活动一秒共 120 帧，帧间隔中位数
`8.334 ms`、P95 `9.611 ms`、最大 `12.933 ms`。桌面模拟器、原速／慢速三列视频、完整关键帧和开场边缘
放大对比均已更新。

已通过 `:app:publishDebugUpdate` 发布 Debug 更新 `202608291731`。本地与远端
`latest.json` 的 SHA-256 均为
`543b2f88803bdc5dfac221e0851343c486f87175a72b92fa512e67ee3c2a6a20`；本地版本化 APK、
远端元数据记录值和远端 APK 回读文件均为 `24,404,408` 字节，SHA-256 均为
`4cd8edbf93cc4728f97f602a6dff8d6d09b9d1ad7db5ae5eaffeeac53b78ec12`。

## 2026-08-28 - 连续布面 A 迁移 Android：单一折叠脊与有界尾流

用户先在桌面交互网站确认 A 的连续布面方向“看着还行”，本轮要求迁移到 Android 实机
查看。需保留的关键约束包括：开场 Dialog 未脱附主体不整体平移；粒子化与局部拉伸同步；
帷幔是一条连续、真实更密的折叠脊；彩色只增加同一粒群的真实点数；末段顺风为主，不得向
四面八方扩散。

`ParticleDismissRenderer.kt` 已将桌面 A 的视觉约束重写到 canonical Android GLSL。静止快照与粒子层
现在共用连续 `curtainActivationField`：定向主体场、两个上游边缘出口和一个下游角出口只负责
开场多处近同时解除；中段密度与输运在 `t=0.24–0.46` 逐步合并为单一全局弯曲折叠脊，不再为
每个出口保留独立高密条带。末段删除随时间继续放大的随机横移，改为单次材料低频噪声与解析
正弦组成的有界连续流；所有顺风响应为正。

本轮同时处理了上一 Android 版本的卡顿风险：删除两组 curl 有限差分的多次噪声采样；普通尾流
只全量保留主体层，前后层各以 `0.12` 概率保留，折叠脊才恢复三层。按层数反推占用率后，折叠/
尾流的理论真实粒子数目标仍为 `4.2×`。彩色只进入 `keepProbability`，不进入释放、运动、寿命、粒径、
透明度、材质或三层公式。

验证直接从 `ParticleDismissRenderer.kt` 提取 Android Shader 离屏编译与渲染，不维护第二份模型。
`check_color_emphasis.py` 通过：彩色 alpha 占比为源图的 `2.16×`，真实点占比为 `1.39×`；
折叠脊/尾流点占地密度为 `2.44×`，三层联合支撑为主体层的 `1.93×`，前后层质心距为 `33.44px`。
`analyze_raster_consistency.py` 已验证桌面与 Android 等效高像素密度的统计一致性；真实 `glslangValidator`
编译回归已通过。本轮不使用 ADB，真机视觉和帧率需用户手动安装后连续录屏复验。

已发布 Debug 更新 `202608280944`，APK 为 24,404,408 字节，SHA-256 为
`d417013983687dbcea3223978607060b00d0e5a026143c18198c4ccbe06c72bf`。远端 `latest.json`
与本地文件逐字节一致，远端 APK 回读后与本地发布产物 SHA-256 一致。

## 2026-08-27 - 多起点帷幔恢复真实内容色强调

用户发现第四十二轮桌面视频重新以白色/中性色粒子为主，并指出此前提交已专门实现内容色
强调。canonical Shader 原始离屏回归复现：测试快照高饱和 alpha 占 14.79%，粒子阶段
只占 19.71%，强调倍率 1.33×。单变量探针：关闭提亮无变化，尺寸/寿命增强仅 1.40×，
恢复高饱和内容色专属的两个副本达到 2.76×，确认重写消失 Shader 时误删彩色副本为根因。

修复后，白色面板和黑字仍为单颗，高饱和控件/图标/标题获得两个较小副本；三颗粒子共享
局部激活场和前段帷幔运动，仅切向错开不到一个 cell，中后段才轻微分离。同步恢复内容色
尺寸与长尾概率权重，RGB 继续直接继承快照。新增 `check_color_emphasis.py`，以 4 种子 ×
4 时刻检查 canonical Shader，要求强调倍率 ≥2.0×；修复前 1.33× 失败，修复后 2.82×
通过。单次、12 种子、四方向视频重新生成；Kotlin 编译与单元测试通过，未使用 ADB。

发布号 `202608270805`（任务按 UTC 命名），APK SHA-256
`ee8c7b07ddf8e364fc82bc4e0d2169a677b8afa71b2c3b83bd2dfaa17499f1e0`。
日志：`docs/features/dialog-particle-dismiss/debug-updates/update-20260827160521.md`。
远端 latest.json、本地 APK 与重新下载文件的 SHA-256 已核对一致。

## 2026-08-27 - 多起点连贯帷幔消散模型

用户提供 HarmonyOS 7 清空通知屏摄视频，要求重新分析当前 Dialog 粒子动画为何缺少轻盈、
灵动、优雅感并实现修正。逐帧复核确认：参考画面由多个局部锋线近同时掀起，不是一个
扫描线；相邻粒子早期保持片层，之后才松散。讨论中用户依次纠正并确认：

- 消散方向跟随触点，点下方向下、点左方向左；无触点默认向上，不能固定上扬；
- 起点**主要但不是全部**位于飞行反向侧，侧边和少量顺向侧也必须参与；
- 多个起点近同时出现，允许受控随机；
- 未被帷幔触及的区域保持完整，不接受动画开场整卡全局打孔；
- 本轮只重做消失，出现动画保持现状；主体约 0.85s、尾粒子最晚 1.0s。

代码重构：`ParticleDismissRenderer.kt` 将出现/消失拆为独立 Shader；新消失 Shader 用五个
受约束边缘种子（3 上游、1 侧边、1 顺向）生成弯曲局部锋线，静止层与粒子层共享同一
`curtainActivationField`。同一帷幔前 120–200ms 使用共享位移、折叠和低频流场，个体扰动
后置；移除早期全角度随机、强 `pinch`/`flare`、消散三副本堆亮、3%–9% 全局抽稀和极短
闪现。`ParticleDismissController.kt` 保留精确触点方向，不再随机旋转主方向。

桌面验证重做：新增 `tmp/particle-dismiss-tuning/render_curtain_model.py`，直接提取 Android
canonical GLSL 编译，不再维护第二份 Shader；输出单次 60fps、12 种子矩阵、上/下/左/
右上方向矩阵及参考视频并排对照。旧模拟器已标记为历史模型。验证：
`:app:compileDebugKotlin`、`:app:testDebugUnitTest` 通过；canonical GLSL 在桌面 OpenGL 3.3
编译并完成全部离屏渲染。未使用 ADB，真机材质与性能需安装本调试版后目检。

发布号 `202608270727`（任务按 UTC 命名），APK SHA-256
`0300e305cdc1fcc19c56e732876b82ca10201b7c0d77025fb8e89de046545afe`。
发布日志：`docs/features/dialog-particle-dismiss/debug-updates/update-20260827152537.md`。
远端 latest.json、本地 APK 与重新下载文件的 SHA-256 已核对一致。

## 2026-08-27 - 局部帷幔 + 破碎轮廓 + 碎化即飞散（第三十五~四十一轮）

发布号 `202608270402`（任务按 UTC 命名），APK SHA-256
`df384dd53a186c449253ced749711dc8c63402ed03161bb374ec07ee19f7b15f`。
日志：`docs/features/dialog-particle-dismiss/debug-updates/update-20260827120208.md`。
远端 latest.json 与本地 APK SHA-256 已核对一致。

用户连续三轮反馈：(1) 碎化与消逝分成两段、中间有静止分割线；(2) 轮廓
一直方方正正、总是从一条边出发；(3) 浓带该是"一条边上的一部分粒子形成
的帷幔弧"、要更夸张更浓。落地：
- **碎化即飞散**：删除悬浮态（粒子碎化后等揭开线的阶段），粒子在自己
  delay 起飞；"开始就露背景"改由抽稀格子在 delay×0.15 提前从静止层
  擦除承担，两过程重叠。逐帧变化量曲线单峰无低谷 = 无静止段。
- **轮廓破碎**：边缘不再进 w（0.85→0.10），不再豁免抽稀；四周 edgeErode
  （13% 内、边界 ±10% 噪声摆动、强度沿边起伏）同时向内侵蚀。
- **局部帷幔**：沿揭开起始边随机选一段（curtainCenter 0.2–0.8、半宽
  0.13–0.20 → 段宽 26–40%），段内全保留 + 副本全开 + 尺寸 ×2；段中心
  先掀两端拖后（curtainArc 0.07）成下垂弧；整条揭开线另有大弧 ARC_BOW
  0.20（往上飞波峰/往下飞波谷、弧心每次随机）。时间预算重排 sweep 0.24
  + arc 0.20 + warp 0.14 + jitter 0.04 = 0.62，总时长 1.345 守恒。
- 抽稀格子改为"揭开线扫到瞬间闪现"（lifeMod 0.07）——浓带是全格子密度、
  带外只剩保留粒子，浓稀对比来自数量。
- 排障：静止层 uPanelColor uniform 声明被早前批量替换误删 → shader 编译
  失败 → 整层降级淡出（真机表现为"面板整体变暗、无粒子"），加异常日志
  一轮定位。教训：GLSL 批量文本替换必须 assert 命中数。
- 三星验证：f008/f009 局部一块不规则起碎、随后整卡稀疏透出底下界面、
  帷幔聚成明亮浓带飘起；凝聚（uSweepTime=0 门控）不受影响。工作区未提交。

## 2026-08-27 - 悬浮粒子场 + 弯曲断续浓带（第三十三~三十四轮）

发布号 `202608270235`（任务按 UTC 命名），APK SHA-256
`0850eaa01de679a0302fadd77962a796652ba7958be564bdc3706da3450d6727`。
日志：`docs/features/dialog-particle-dismiss/debug-updates/update-20260827103503.md`。
远端 latest.json 与本地 APK SHA-256 已核对一致。

用户两轮修正：(1)"稀疏露背景的背景 = dialog 底下的内容"——碎化要在
**动画开始时**全局发生（tShatter = shatterN×0.10 + h1m×0.05，凝聚
mix 回 delay），整卡 ~0.15s 内化为悬浮粒子场（白底 keepBase 6-18%
随空间起伏、原位噪声漂浮 floatDrift、高权重副本悬浮期也显示成小簇），
静止层同步在 tShatter 擦除露出底下界面；揭开线扫到才掀起飞散。(2)
浓带太规整/四边不该同处理/一条边内各段不该一样——edgeBoost 加
edgeWave（±0.08 噪声摆动边界线，带呈波浪）与 edgeMod（沿边低频
强度 0-1，段落断续；四边采样噪声不同自然各异）。浓带定义最终版 =
**边缘与内容色转换出的密集粒子群**（w 含 0.85×edgeBoost；w≥0.78
副本全程 ×3，黑字 0.62 除外——用户裁定保留）。真机 f033 手动指认：
顶缘带 0.376 / 右缘带 0.327 / 稀疏悬浮场 0.042，比值 8-9；弯曲断续
四边各异逐帧确认。中途弯路：自动检测两次把卡片本体标成浓带（检测
先分割本体再找带）；横向框套竖直带（带方向要先目检）。工作区未提交。

## 2026-08-27 - 白底抽稀：浓稀对比内生（第三十二轮，用户提议）

发布号 `202608270010`（任务按 UTC 命名），APK SHA-256
`7e1c0e9149027e70b77bc6561a5c6a4a038346feb863468f582e70efc6928fff`。
日志：`docs/features/dialog-particle-dismiss/debug-updates/update-20260827081014.md`。
远端 latest.json 与本地 APK SHA-256 已核对一致。

用户洞察：华为浓带成立是因为别处稀疏——通知卡是大部分透明的毛玻璃，
透明区产生的可见粒子极少。量化证实：卡底不透明度 α≈0.26（卡区 136 /
壁纸 95 / 白 255 反推），可见粒子天然集中在内容与边缘。用户提议对
不透明控件等效：白底大部分区域直接少转化粒子露背景、边缘与彩色区
多转化。实现：主 hash 第二随机数 h2m（主/副本一致）判定 culled——
keep = max(0.30, w, keepEdge)，keepEdge = smoothstep(0.10, 0.03,
edgeDist)；culled 格子激活即 degenerate（静止层未擦前仍完整显示）；
仅消散（step(0.001, uSweepTime) 门控），凝聚不抽稀。蓝本量化：带/
已飘散比 19.8–122.8（前版 6.2–29.2 再上台阶），彩色团饱满、白稀
分明。三星逐帧：边缘一段起碎、弧线推进、碎屑带+彩色浓团 vs 稀疏区
结构鲜明。工作区未提交。

## 2026-08-27 - 量化对照华为重做揭开与浓带（第二十九~三十一轮）

发布号 `202608262225`（任务按 UTC 命名），APK SHA-256
`5de43bc18f6685287b25dbd3b02cad5a65d6f60369396dd444679ddc49ef53d0`。
日志：`docs/features/dialog-particle-dismiss/debug-updates/update-20260827062435.md`。
远端 latest.json 与本地 APK SHA-256 已核对一致。

用户严批"自以为实现了浓带"，要求先量化华为再逼近、并在截图中指认
浓带。量化华为清空段（block_std 剖面）：浓带宽 ≈30% 跨度、峰/尾密度
比 4–8、单向扫过 0.44s；我方旧版实测比值 1.0–1.2（无带）。三轮重构：
(1) 单向揭开——delay 主项 = 沿飞行方向投影（uSweepDir CPU 算、
uSweepTime 0.35，凝聚传 0），揭开线从飞行反侧边缘扫向前侧（"揭
便利贴"用户模型）；(2) 大振幅弧线锋线（WAVE_WARP 0.22、波长 2.2×
noiseScale）——起始只从边缘一段掀起，弧线一头密一头稀；localEdge
项让其他边缘噪声谷段同步局部掀起；(3) **带内副本**（浓带的数量
来源）——所有粒子 tl<0.30 时激活副本（×3 聚在揭开线附近、
0.18-0.30 淡出还回单颗），叠加辉光/尺寸+35%/ease 1.6 前慢。中途
弯路：整卡颗粒态方案被用户否（浓带必须是纯粒子、未掀开区保持
清晰）已删；寿命两极 bipolar 门控仅消散（凝聚倒放会出黑洞）。
蓝本 bug 修复：set_still 的 warp/jitter/spread 未读场景覆盖值（凝聚
两层错位、静止层全黑）。量化验收：真机 f038 暗背景纯粒子带亮点
密度 0.608 vs 已飘散区 0.104（比值 5.9 ∈ 华为 4–8）；蓝本带/已飘散
数量比 6.2–29.2；标注图 dev_band_final2/band_mark3 红框可指认。
三星+OPPO 双机形态：起始边缘一段、弧线推进、浓带随线移动。
工作区未提交。

## 2026-08-26 - 锋线浓带：寿命两极分化 + 平台式衰减（第二十八轮）

发布号 `202608261543`（任务按 UTC 命名），APK SHA-256
`372e4edb6221e4aebe498879501372fff867939b07bdaa19c0df43dad1f36848`。
日志：`docs/features/dialog-particle-dismiss/debug-updates/update-20260826234314.md`。
远端 latest.json 与本地 APK SHA-256 已核对一致。

用户："根本没有变厚，所有地方粒子密度一样"——附华为截图红框：锋线
弧线带浓、其余稀疏。复盘：第二十七轮把"厚"做在**空间固定**的源头区
（方向错——华为的浓带**跟随锋线移动**），且 alpha 渐降的浓稀对比仅
~2:1 不可感知。重构为**数量对比**：(1) 寿命两极分化——2/3 粒子短寿
0.30–0.55（只活在锋线带、组成浓带后消失），1/3 长寿 0.75–1.25（稀疏
飘全程），h6 = fract(h3+h4·0.618) 门控，内容色长寿概率 32%→67%
（余缕仍彩色主导）；(2) fade 平台式——前 55% 寿命满 alpha、0.55-0.95
快速谢幕（替换渐进衰减）；(3) density 淡入延后 0.05→0.30（带内不被
打薄）。浓带=全数量满 alpha，带后=数量 3:1 骤降。上限 1.25/总时长
不变。三星逐帧：浓带随锋线翻卷、已扫区明显稀疏；OPPO 中后段稀疏
彩色残缕正常。凝聚呈"稀纱渐浓、凝实前沿最亮"递进。工作区未提交。

## 2026-08-26 - 边缘掀起 + 掀起点加厚（第二十七轮）

发布号 `202608261530`（任务按 UTC 命名），APK SHA-256
`89343447063b0ceba3dc10850eae96f3c16d539beddceaf1e9afe63ec2910fe6`。
日志：`docs/features/dialog-particle-dismiss/debug-updates/update-20260826232952.md`。
远端 latest.json 与本地 APK SHA-256 已核对一致。

用户两点反馈：起碎点太多且出现在 dialog 内部（应从边缘起碎）；华为
掀起薄纱的起点粒子更浓厚。落地：(1) delay 加 edgeDist×EDGE_DELAY
（0.32，中心比边缘晚 0.16s）——起点只出现在边缘与噪声谷交集，内部
不再冒点、起点数自然收敛；SPREAD_TIME 0.20→0.06（触点趋势近乎无）、
WAVE_WARP 0.30→0.28 配平，delay 上限 0.62、总时长 1.345 精确守恒。
凝聚传 uEdgeDelay=0 保持斑块凝实。(2) originBoost = 1-smoothstep
(0.08, 0.30, delayMain) 标记起碎源头，副本门控放宽——源头区白色副本
也激活（粒子数真实 ×3，掀起的烟一路更浓）。粒子层与静止层同款公式。
三星起碎三帧：只从边缘掀起（四段不连通）、内部完整、掀起带浓厚向内
翻卷；OPPO 同形态正常。工作区未提交。

## 2026-08-26 - 真正的多点起碎：双尺度锋线噪声 + 谷整形

发布号 `202608261513`（任务按 UTC 命名），APK SHA-256
`d6f707bf245ae6d8e777c9b4c62c14c5c34234e9a653de66481ed43c3717d8ad`。
日志：`docs/features/dialog-particle-dismiss/debug-updates/update-20260826231255.md`。
远端 latest.json 与本地 APK SHA-256 已核对一致。

用户指出上一版"并没有从多个点开始，都聚集在某个区域"。根因：锋线
噪声单一波长 192dp，对话框尺度内只有一两个低谷。修复：waveWarp 改
双尺度——`0.45×vnoise(1.6×scale) + 0.55×midN`，中频项波长
0.45×scale≈66dp 且做谷整形 `midN = 1-(1-v)²`（高值区抬平、低谷保留
——多数区域晚碎、分离的谷显著提前冒出）。粒子层与静止层同款公式
（擦除对齐）；蓝本先行两轮（首轮 0.55/0.45 无整形分离性不足）。
三星录屏起碎早期帧：5+ 处互不连通起碎区同时开花、扩张合并成波浪
锋线；OPPO 0.3s 撞帧：6-7 处分离斑块。凝聚对称呈现多区域同时凝实、
起点散云无退化。总时长不变。工作区未提交。

## 2026-08-26 - 薄纱质感四项升级（华为对照第二十五轮）

发布号 `202608261455`（任务按 UTC 命名），APK SHA-256
`31bf3b1492078ab2fc335814c2de7c2a740b51c026807e61ae8fa2bd69a598f5`。
日志：`docs/features/dialog-particle-dismiss/debug-updates/update-20260826225429.md`。
远端 latest.json 与本地 APK SHA-256 已核对一致。

用户给华为动画 mp4（第 5 秒起），逐帧解构出四个差距因子并全部落地：
A 波前重构——锋线扭曲改单侧 [0,0.30] 噪声主导 + 波长 1.6×noiseScale，
spread 0.40→0.20（时间预算转移，delay 上限与 TOTAL≈不变 1.345s）——
锋线成大波浪、多处同时起碎（华为侵蚀式）；B 细密化——CELL_DP 1.5→1.1
+ 尺寸曲线 0.9/0.32（共约 -34%），MAX_PARTICLES 150k→220k（顶点 66
万）；C 锋线辉光——litRgb = mix(color, min(color×2.2, 1), 0.75×glowEase)，
glowEase 前 45% 寿命衰减完，用户裁定"原色的发光版本"非固定银色；新
varying vGlow 控 fragment 软边内径 0.30→0.12（光晕）；D 浓度梯度——
fade 终点/指数按 w 插值（白底 0.76/2.0 快净、内容色 0.92/1.7 余缕
全程）。静止层同步单侧 warp 公式（擦除对齐）。蓝本三轮调参（首轮
整体消失过快，D 改为只作用低权重）。三星逐帧：波浪锋线白亮细纱、
扫过即净、彩色余缕收尾；凝聚凝实前沿闪光；消散段 138 帧=动画期满
60fps 无掉帧。OPPO：Adreno 正常。工作区未提交。
...
6b2a0b8c958c15ef4eade0ee147af1d983963c3a1c915726b1df8dda26b5ba08（22,160,344 字节）。
本地 `app/build/outputs/update-debug-apk/latest.json` 与远端 `releaseNotes` 逐字一致
（1388 字，单 `## ` 节，首尾片段均在，未截断）。
日志文件 `docs/features/fablesol-video-export/debug-updates/update-20260730163251.md`。

- 内容：评审 29 项修复（失败 Dialog/通知的「调整导出设置」、HDR10+ 膝点门禁假阳性、
  参考峰值不可行专用类别、HLG 回环形态贯通与瞬时失败不缓存、FP16 回退同步完成信息、
  蓝噪声回退门、复杂度/B 帧/CQ 形态三条兼容阶梯、Level 码率余量、设置页 B 帧原因与
  信号范围预计落点、13 套语言若干新增文案）+ 7 项待裁定裁定（D185 条件承袭、D186
  运行时形态阶梯、SEI 逐样本计数、pack16 上取整等）。
- 提示：`PROBE_CONTRACT_VERSION` 13→14，装上后首次打开设置页会整份重探矩阵（预期）。
- 发布前验证：FableSol 76 套件 545 例强制重跑 0 失败；未使用 adb，未安装到物理设备。

## 2026-07-30 - 说明行随选项显隐；让步顺序把编码模式排到帧率之后（D184）

发布号 202607300209；APK SHA-256
0b5729c83f72a6a5d94cba0d7b81bf373229d491b4755352e5e487696d3cad47，22,120,540 字节。
本地 APK、`app/build/outputs/update-debug-apk/latest.json` 与远端三处一致，远端
`releaseNotes` 1032 字，两部分与「请确认」段都在（单 `## ` 节，未被截断）。

日志文件 `docs/features/fablesol-video-export/debug-updates/update-20260730100836.md`。

- 恒定质量下「复杂帧质量保护」那一行是隐藏的，说明却无条件写入。已改为与选项共用
  `prefersConstantQuality` 判据。全面核对这一组十一行说明，仅此一处；完成信息一侧本来就正确。
- 设置页修正不成立组合时，编码模式此前不在枚举里而被当成硬前提，于是 120 fps 无 CQ 通路的
  设备上「恢复默认」会掉到 60 fps 恒定质量。现让步顺序为编码模式 → 编码器族 → 输出格式 →
  帧率（权重 1/2/4/8）；同时修正了原先「宁可降帧率也保住格式」与 D179 相反的权重。
- 全量单测 78 套件、544 例、0 失败、1 跳过；未使用 adb，未安装 APK。

## 2026-07-30 - 修复恒定质量实际导出为 VBR

用户反馈编码模式明确选择“恒定质量”，导出成功 Dialog 却显示 VBR。诊断确认 Dialog 显示的是
实际码流结果，错误位于设置与候选解析：设置页原先用设备上任意 60 fps 编码路径的 CQ 能力启用
选项，没有携带当前 HDR 格式、编码器族、位深和 120 fps；当前精确档位不支持 CQ 时，候选解析
又会将其静默转换为 VBR。

本次修复：

- `FableSolExportCapabilityMatrix` 增加编码模式维度，CQ 与目标码率分别按同一完整规格短编码验证，
  并共同保存在一份能力矩阵中。
- `FableSolExportEncoder` 排除不支持所选编码模式的候选；`FableSolExportRateControlForm`
  不再允许 CQ 解析为 VBR。
- `FableSolTuningDialogFragment` 删除代表性 CQ 区间查询，编码模式与格式、编码器、位深、帧率
  统一联动；质量滑杆读取当前具体编码器实测区间。
- 编码模式成为第六项公开规格，自动重试不得在 CQ 与目标码率之间切换。无精确组合时使用正式
  本地化说明阻止导出；公开规格和无解文案已同步 13 套语言。
- 新增原始失效模型、严格模式、矩阵序列化、公开规格及本地化回归测试。

发布日志：
`docs/features/fablesol-video-export/debug-updates/update-20260730094211.md`。
发布前验证为 78 个套件、542 例测试、0 失败、0 错误、1 例跳过；13 份字符串资源均可解析，
debug APK 构建成功，`git diff --check` 无格式错误。未连接或安装到物理设备。

首次发布 `202607300139` 后复核发现，完整组合无解时设置摘要仍会附加“· …”或目标码率估算；
修正为仅显示正式原因并重新执行全量验证。最终已通过 `:app:publishDebugUpdate` 发布阿里云
debug 更新 `202607300142`。本地发布副本、远端 `latest.json` 与下载后的远端 APK 均为
22,120,540 字节，SHA-256 均为
`826099d35c8507088a5f9477573828208a3796a9d4d73091353273746e16fb5b`；发布说明逐字一致。

## 2026-07-30 - 严格导出规格、OPPO HDR 验收与软件 AV1 画质修复

用户先要求修复三星 Z Fold4 选择 120 fps 后，自动结论和实际导出仍落到 60 fps HDR10／
AV1 的规格不一致；随后逐项裁定公开规格变化必须确认、同规格内部重试的说明边界、前后台
确认方式及正式文案要求。实现中将 60／120 fps 改为严格规格，统一设置页三轴解析与运行时
候选；输出格式、编码器族、位深、硬件／软件类型或帧率变化均进入 Dialog／通知确认，后台
通知正文直接采用建议规格且与操作按钮共用幂等命令。失败原因、当前规格与最近一次调整进入
重试、成功和最终失败状态，全部补齐 13 套语言。

用户随后授权连接 OPPO `3B1629006YC00000`，要求以记事“测试音频呀”的
`20260729001238.wav` 导出多种 HDR／SDR 组合，并用 FFmpeg、FFprobe 和手机截图复核亮度
稳定性、高光、水体颜色与画质。样片与分析结果保存在
`E:\projects\fablesol-export-samples-20260729`。

- HDR10+ 的 D177 场景级实现通过 203／350 尼特实测：两份新样片均为 853 帧、1 份唯一
  ST 2094-40 载荷；高光活跃时水体没有压暗。D176 的母版峰值横轴确认无需保留。
- HDR10、HLG 扩展／名义范围、杜比视界 8.4 及多种 SDR 组合完整解码，未检出高频明暗变化，
  水体色相连续且高光保留。
- 发现 `c2.android.av1.encoder` 的最高 CQ 仍在 HDR 水体渐变中产生明显量化块；VBR 对照
  未改善。最终仅对 AOSP 软件 AV1、HDR、最高 CQ 且支持 QP bounds 的路径增加 QP 上限 8，
  码率由约 580 kbps 提升至 1.151 Mbps，同帧接触表中的量化块消失，公开规格不变。
- 修复设置页 CQ 能力查询遗漏 AV1；将实际为 VBR 的模式从“恒定码率”更名为“目标码率”，
  完成 13 套语言国际化及语义回归测试。

发布日志：
`docs/features/fablesol-video-export/debug-updates/update-20260730085240.md`。
发布前验证为 78 个套件、539 例测试、0 失败、0 错误、1 例跳过；debug APK 构建成功，
`git diff --check` 无格式错误。

已通过 `:app:publishDebugUpdate` 发布阿里云 debug 更新 `202607300053`。本地组装 APK、本地
发布副本与远端 APK 的 SHA-256 均为
`f05c7cecc460fcc2e536a285150c094b4f61f3c94e334c3a483aeddb4b491d1e`，文件大小均为
22,119,804 字节；远端 `latest.json` 的发布号、SHA-256、大小与发布说明均和本地逐项一致。

## 2026-07-29 - 视频导出画质整体重做（八批 D62～D178）

发布号 202607291308；APK SHA-256
ea210c8092d11e38e5ad600491ede4a7eb2844cf0de60104f29367d1d17b6882。本地、
`app/build/outputs/update-debug-apk/latest.json` 与远端三处一致，远端 `releaseNotes` 2525 字。

日志文件 `docs/features/fablesol-video-export/debug-updates/update-20260729210641.md`
全文写在单个 `## 本次更新` 节里，避免 2026-07-25 那次分节被静默丢弃的问题。

- 设置侧：色彩模式七档且 HDR 在前、SDR 分原生/保留高光两种语义、SDR 位深三档、HLG 信号范围
  两档、PQ 漫反射白回到标准 203 尼特、HDR10+ 参考显示峰值滑杆、编码模式与 B 帧等画质选项成组、
  每个选项的说明改为紧跟其下常显。
- 内部：应用自有 P010 成为 10-bit 首选输入（FP16 中间面、色度位置匹配、闭环亮度修正、蓝噪声
  量化）；MaxCLL/MaxFALL 编码前全片实测并按渲染指纹缓存；HLG 逐颜色方向色容积 + 真实编解码
  回环验证超白。
- 修复用户反馈的 HDR10+ 亮度往复变化：改为整段动画按一个场景统计，只生成一份 ST 2094-40 载荷
  逐帧重复（D177）。
- 代价：导出时长增至两到五倍（同素材 8-bit 表面 12.1 s vs HDR10+ 首次 68.2 s）。
- 未安装到设备、未生成新样片；OPPO 上 203/350 尼特 HDR10+ 的实际观感与设备矩阵剩余项待用户复核。

## 2026-07-28 - 位深进入结论并显示

发布号 202607280137；APK SHA-256
43eb602718ab08990bbe6c4db97a2f8cfdbc68eef52510282c01bd0d436e8c14。

SDR 阶梯里 HEVC 本来就有 10 位与 8 位两档，10 位在前。华为平板上 10 位不产出样本，此前被判
为通过，8 位那一档因此从未被尝试。上一版补上样本门之后，该机会自动落到 HEVC 8 位，仍可
120 fps，不必退到 H.264。

- 位深进入可行组合表并跨缓存保留；推导文字与完成信息写成「HEVC 10-bit（硬件编码）」。
- 策略维持自动：优先 10 位，编不出来才退 8 位（8 位档启用抖动）。10 位更不易出色带，但分享
  兼容性明显差于 8 位——是否要开放为用户可选项待定。

## 2026-07-28 - 补上「必须真的产出视频样本」这道门

发布号 202607280124；APK SHA-256
c1e8e22ac38e8c351ae951534733f352af2b9d20387e4d3b549809782ded0b7d。

华为平板上每一档 HEVC 都导出成 0 字节，SDR 与 HDR、60 与 120 fps 皆然，只有 H.264 有数据。
判据不完整所致：`INFO_OUTPUT_FORMAT_CHANGED` 一到就能 addTrack 并启动 muxer，此后一个实际
样本都不产出，`finish()` 照样成功返回。探测与导出都以此为准，于是不报错也不产出的编码器被判
为通过。

成因：该机带 `EGL_RECORDABLE_ANDROID` 的 10 位窗口 config 为 0 个，编码器消费不了这些缓冲。
SDR 通路的首选档是 HEVC Main10，同样走 10 位表面，所以关掉 HDR 也躲不开；H.264 是 8 位档，
走另一条 config。

- 以实际写入的视频样本数作为硬门禁，探测与正式导出两处都验。
- 样本计数正常但落盘为空时，发布前再核对一次文件大小。
- 任一不通过即判该候选失败，阶梯换下一档，该机会自动落到 H.264。
- 探测契约版本升至 10。

## 2026-07-28 - 修复完成通知闪退；编码器说明写明落点

发布号 202607271718；APK SHA-256
c2344501aeef36b51c1ac3a58c4f7babea454ca89fe0485cb47c890fbe51e219。

- **华为平板导出完成后闪退**：通知栏分享按钮用 `Intent.createChooser` + `EXTRA_STREAM` 构建，
  `PendingIntent.getActivity` 会在**构建时**立即走一次 URI 授权，该机拒绝并抛
  SecurityException（`UID … does not have permission to content://media/external/video/…`）。
  视频已写完入库，却死在发通知这一步。现在授权失败只省略分享按钮；打开跳转与 notify() 也各自
  兜底。对话框内的分享走 Activity 上下文，不受影响。
- 编码器胶囊说明改为随选择变化：档位特点（HEVC / AV1 / H.264 各自独立）、当前实际使用的编码器
  并标明硬件或软件、灰色选项的含义。随格式与帧率变化重新解析。
- 推导文字结论行加粗并与推导过程空一行；推导过程与设备能力报告之间不再留额外间距。

## 2026-07-28 - 三条轴互相约束；profile 未回报不再判死

发布号 202607271653；APK SHA-256
f7d56d49795e4e243f3b44b9e6e92d4f9be74f7cbef78b7d02815bd742c5e7d9。

- **华为平板的 HDR 是被自己的校验挡的**：编码器未在 outputFormat 回报 `KEY_PROFILE` 时，
  此前一律判为「降了档」（`Encoder changed profile 2 to null`）。改为与色彩键一致，缺失即
  没有信息。该机 10 位表面本已产出输出，很可能因此恢复 HDR10。
- 帧率、导出 HDR 格式、编码器改为三向互锁，帧率不再是「上限，不行就自己降」。冲突时按
  「保住刚变更的那条、其余改动最少」枚举出成立的组合。
- 恒定质量档要求该编码器能处理本次画布（Z Fold4 上支持 CQ 的编码器上限仅 512×512）。
- 「帧率上限」改称「帧率」；推导文字与完成信息同时标注硬件/软件编码。

## 2026-07-28 - Z Fold4 的 10 位通路确认无解；补记平台能力位

发布号 202607271616；APK SHA-256
1482393cfd39f3aa2b0861bb68f8c4c2fd376511ca5a6c653ef5b49378ed2e51。

枚举结果确定：该机共 4 个 10 位窗口 EGL config，**带 EGL_RECORDABLE_ANDROID 的为 0 个**。
上一版的枚举修复对该机无效，`eglChooseConfig` 此前的结果是准确的。加上编码器不支持 P010
字节缓冲输入，该机硬件 HEVC 在公开接口上没有任何可用的 10 位输入通路，HDR 只能由软件 AV1
承担。这是设备限制，不是缺陷。

- 新增「10-bit 表面编码能力」一节：`COLOR_Format32bitABGR2101010` 输入、`hdr-editing`、
  `hlg-editing`。仅作记录，判定仍只认真实编码。
- 同一编码器族内只重试一次，避免在注定失败的组合上反复等待。
- 失败原因中的换行压平。

## 2026-07-28 - 定位到 EGL_RECORDABLE_ANDROID；能力报告可复制

发布号 202607271607；APK SHA-256
afb5e4db77c8c8a02637b6639384fb5ce5a3945b74a22739d0b2ac7ce016c7aa。

三星 Z Fold4 上 SDR 通过的是 **HEVC Main SDR**（8 位），10 位的 Main10 同样失败，而
「10-bit 窗口配置」停在不带 recordable 的那一档。据此定位：`EGL_RECORDABLE_ANDROID` 决定
缓冲带不带视频编码器用途位，缺了它 MediaCodec 输入表面的缓冲编码器消费不了，10 位各档因此
一律以编码器「已被释放」告终。此前代码把它当成「给驱动的提示」直接降档，判断有误。

- EGL config 选择改为在 `eglChooseConfig` 落空后自行枚举全部 config 并用
  `eglGetConfigAttrib` 逐项核对：该属性是厂商扩展，部分驱动不纳入匹配却读得到。
- 能力报告新增 10 位 config 清点（总数 / 带 recordable 的个数）。
- 设置页的推导文字与设备能力报告支持长按复制两段全文，新增提示字符串（13 套语言）。
- 探测契约版本升至 9。

## 2026-07-28 - 区分位深落点；编码器状态类错误改为重试

发布号 202607271555；APK SHA-256
d692aa2edd5b996a4dd01bb5f97b8f49f4eb82ea3bd1ae32ea93739d3175068e。

三星 Z Fold4 上 SDR 已恢复硬件编码（HEVC 120 fps、H.264 120 fps），上一版把 8-bit 档输入表面
位深改为跟随档位起了作用。HDR 各档仍失败，报错是编码器状态问题而非参数不成立：
`Invalid to call at Released state`（HDR10+ / HDR10）、
`Pending dequeue output buffer request cancelled`（HLG）。两者都不是 CodecException，
上一版依据 isTransient 的重试一次都没有触发。

- 重试判据不再只看 isTransient，同时按错误消息中的状态特征识别；失败候选之后加入 80ms 沉降。
- 能力报告写明通过验证的阶梯项名（HEVC Main10 SDR 与 HEVC Main SDR 分得开），用于区分
  「10-bit 编不了」与「仅 HDR 信号编不了」。这是下一步判断的关键分叉。
- 探测契约版本升至 8。

## 2026-07-28 - 修正探测链路，补齐编码器失败原因

发布号 202607271543；APK SHA-256
be99c9c0b20f51f5260f84b741ad6e97316a06a274aacd62ce5c896c722a8c95。

三星 Z Fold4 上四个高通 HEVC 与若干 H.264 全部不可选，SDR 也落到软件 AV1。本次修正探测自身
的两处问题并补齐判断依据：

- 能力探测创建输入表面时位深写死 10-bit，8-bit 档（H.264、HEVC Main）验的与正式导出用的不是
  同一条链路。改为跟随档位。
- CodecException 的消息常为空，诊断里只剩一句 IllegalStateException。现在一并输出厂商诊断串、
  错误码与瞬时/可恢复标志；瞬时错误重试一次（探测会连续创建释放数十个编码器实例，硬件实例
  有限）。
- 诊断新增两节：被淘汰的编码器组合及原因（此前只有整种格式失败时才保留），以及每个编码器对
  本次画布的尺寸与帧率答复（用于区分被尺寸帧率筛拦下与编码器自己拒绝）。
- 探测契约版本升至 7。

## 2026-07-27 - 修正推导文字与能力报告（编码器选择跟进）

发布号 202607271503；APK SHA-256
31ceb7301b33fd5a6a71c96867000341d106487aa5da7c6f2753099c62293a43。

- 「自动」格式的落点改为按当前选择的编码器解析。OPPO 上钉定 AV1 后说明文字仍写
  「当前为 HDR10+」，而 AV1 编不出该格式。格式说明、设置摘要与两根 PQ 滑杆的显隐同步修正。
- 体积估算与提示语改用实际可达的帧率；帧率与编码器在同一次解析中得出。
- 能力报告列出每种格式全部可用的编码器及各自最高帧率，此前只显示第一个落点，HLG 因此漏掉
  了 AV1。自动选择一项补上具体落点。
- 冲突迁移改为仅在当前选项确实不成立时才触发，避免把用户明确选定的编码器甩回「自动」。

## 2026-07-27 - FableSol 导出新增编码器选择，修复 HDR 默认被关闭

发布号 202607271443；APK SHA-256
425b4616b2464c0668e7c754a742c1c3f846d8f0e26b4ad5df397ca4e015c54d。

- 设置页不再把「设备编不出 HDR」这个能力结论写进用户偏好。该写入不可逆，是三星 Z Fold4
  上设备能编 HDR10 与 HLG、默认却停在「关闭」的原因。
- 新增「视频编码器」选项（自动 / HEVC / AV1 / H.264），按本机实测的
  (格式 x 编码器族 x 帧率) 可行组合表置灰，并在冲突时自动迁移另一条轴。
- 自动档不再使用软件编码器。Z Fold4 上一次 120fps 的 HDR 导出曾静默落到 AOSP 软件 AV1
  的 60fps 上；软件实现现在需要显式选择，且在诊断、设置摘要与完成信息中标注。
- 能力探测改为按用户实际使用的编码模式验证，探测契约版本升至 6。
...
峰值与最大帧平均亮度；“自动”改为画质/规格优先；顺序固定为
`杜比视界 5 → 杜比视界 8.1 → HDR10+ → 杜比视界 8.4 → HDR10 → HLG`；中文 locale
全面使用“杜比视界”；所有 HDR 能力与失败信息改为专业、严谨、正式的描述。

- `FableSolExportHdrFormat` 分离固定英文 `stableLabel` 与本地化 `displayNameRes`。缓存和
  编码档位保持稳定；设置胶囊、自动结果、能力诊断、完成 Dialog、通知和失败信息统一走
  本地化显示名。
- 杜比视界的传递函数/Profile 不一致改为显示请求值、实际值和目标格式验证结论；各 Profile
  明确独立验证。HDR10+ 改为分别报告未提交与提交 ST 2094-40 元数据的结果，并明确 SEI 判据。
- 设备、EGL、10-bit 窗口和编码器摘要统一使用“支持 / 不支持 / 未发现 / 未声明”等状态；
  探测契约升至 5，使旧口语化缓存失效。
- HDR10、HDR10+、HLG、杜比视界说明和自动漫反射白推导文字已整体专业化；公式分开显示
  原始结果、范围约束、25 尼特向下量化及最终值。
- 定向单元测试、顺序/本地化/正式措辞源码契约和 `:app:assembleDebug` 均通过；未使用 adb。
...
详细日志：
`docs/features/fablesol-video-export/debug-updates/update-20260727164558.md`。

## 2026-07-27 - FableSol 自动漫反射白按屏幕能力与 HDR 强度自适应

用户基于 HDR10+ 800 尼特样片分析要求修改旧的“屏幕峰值四分之一”自动白锚：不得把
350 尼特写成固定值，必须结合每台设备的屏幕能力和当前 HDR 峰值强度计算，并在 FableSol
设置 Dialog 下方的规格/编码推导文字中公开算法。

- `FableSolExportDisplayLuminance` 现在同时校验 `desiredMaxLuminance` 与
  `desiredMaxAverageLuminance`，按
  `floor(min(屏幕峰值×1.75÷HDR强度, 最大帧平均亮度, 400)÷25)×25` 计算，最终受
  200～400 尼特自动范围约束；两项能力都缺失才回退 400 尼特。
- 2000 尼特屏幕、9.6 强度会得到 350 尼特与 3360 尼特母版峰值；350 只是该输入组合的
  计算结果。500～800 尼特仍保留为用户明确选择的手动高亮档。
- 自动档会随 HDR 强度滑杆实时更新白锚滑杆、结果与母版峰值；手动值保持不变，恢复默认后
  才重新进入自动档。
- PQ 规格/编码推导文字新增完整公式，手动档新增状态说明；设备诊断新增屏幕最大帧平均亮度、
  当前 HDR 强度和安全回退标记。13 套语言资源已同步。
- 新增 7 个数值回归与 1 个设置数据流源码契约；定向测试和 `:app:assembleDebug` 通过。
  完整源码契约测试类有一条与本轮无关的既有 Dialog 按钮布局断言失败。未使用 adb。
- 已发布阿里云 debug 更新 `202607270804`；本地与远端 APK、元数据 SHA-256 均为
  `ff8802fef0a1a07089ffe98479e130ae7102e4330c19d7b6ae5621a978eed06b`。

详细日志：
`docs/features/fablesol-video-export/debug-updates/update-20260727160304.md`。
