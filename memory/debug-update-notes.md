...
- 更新 `docs/plans/THING_CARD_MEDIA_TARGET_GEOMETRY_EXECUTION.md` 和 memory，记录 ratio slider 显示策略与 list widget bitmap 保护。
...
- 更新 string resources：
  ...
  - 简中为 `封面图片宽度 · %1$d%%`；
  - 繁中/香港为 `封面圖片寬度 · %1$d%%`。
- 更新 `docs/plans/THING_CARD_MEDIA_TARGET_GEOMETRY_EXECUTION.md` 和 memory，记录此控件是 `sidePanel.targetAspectRatio` 的 alternate projection control，而不是恢复旧的 `sideMediaWidthPercent`。
...
- 更新文档：
  - 新增 ADR 0003 和 `THING_CARD_MEDIA_TARGET_GEOMETRY_PLAN` / `EXECUTION`；
  - 旧的 `THING_CARD_APPEARANCE_PLAN`、`REMOTE_THING_CARD_APPEARANCE_PLAN`、ADR 0002 标记为 media geometry 已由 ADR 0003 supersede；
