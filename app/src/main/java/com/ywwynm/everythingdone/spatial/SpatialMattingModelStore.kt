...
object SpatialMattingModelStore {
    ...
    fun modelDirectory(context: Context, model: SpatialMattingModel): File =
        File(rootDirectory(context), "${model.stableId}/${model.version}")

    fun modelFile(context: Context, model: SpatialMattingModel): File =
        File(modelDirectory(context, model), model.fileName)

    fun isInstalled(context: Context, model: SpatialMattingModel): Boolean {
    ...
    fun isDeviceEligible(context: Context, model: SpatialMattingModel): Boolean {
    ...
    fun hasSufficientAvailableMemory(context: Context, model: SpatialMattingModel): Boolean {
        val manager = context.getSystemService(Context.ACTIVITY_SERVICE) as ActivityManager
        val memory = ActivityManager.MemoryInfo()
        manager.getMemoryInfo(memory)
    ...
    fun installVerified(
        ...
        check(source.isFile) { "matting 模型临时文件不存在" }
        check(source.length() == model.sizeBytes) { "matting 模型字节数不符" }
        check(
            SpatialModelStore.sha256(source).equals(model.sha256, ignoreCase = true)
        ) { "matting 模型 SHA-256 不符" }

        val directory = modelDirectory(context, model)
        check(directory.exists() || directory.mkdirs()) { "无法创建 matting 模型目录" }
        ...
        if (target.exists()) check(target.delete()) { "无法替换 matting 模型" }
        check(pending.renameTo(target)) { "无法原子安装 matting 模型" }
        if (markReady) writeReadyMarker(context, model)
        return target
    }

    fun writeReadyMarker(context: Context, model: SpatialMattingModel) {
        val target = modelFile(context, model)
        ...
        check(pending.renameTo(targetMarker)) { "无法写入 matting 模型就绪标记" }
    }
    ...
    private fun rootDirectory(context: Context): File =
        File(context.noBackupFilesDir, "spatial-photo/matting-models")
