...
internal class BakedBezierInterpolator : Interpolator {
    ...
    override fun getInterpolation(input: Float): Float {
        ...
        val position: Int = min(
                (input * (VALUES.size - 1)).toInt(),
                VALUES.size - 2)

        val quantized: Float = position * STEP_SIZE
        val difference: Float = input - quantized
        val weight: Float = difference / STEP_SIZE
    ...
    companion object {
        ...
        private val STEP_SIZE: Float = 1.0f / (VALUES.size - 1)
    }
}
