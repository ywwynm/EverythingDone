...
**Safe Viewpoint Envelope（安全视点包络）**:
每份 Spatial Photo Effect 逐方向确定的虚拟视点可移动范围；在包络内呈现必须同时保持形状稳定与正确遮挡。用户感知的“可视角度”由三个并列要素共同决定：包络半径（倾斜跟随范围）、包络内的视差响应（等倾斜下前后景相对位移）、遮挡显露（能“绕过”前景看到被遮内容）。
_Avoid_: 把单一数值当作可视角度、把扩大包络等同于放大视差幅度、Spatial Effect Strength（后者是包络内的用户偏好）
...
**释放场**:
对话框表面每一点"在动画的哪一刻脱离本体"的时刻分布；整段消散的宏观走向都由它决定。
_Avoid_: 遮罩、mask、溶解图

**分界线**:
对话框上"已化成粒子"与"仍然完整"之间的那条线，随时间从迎风侧推向顺风侧。
_Avoid_: 侵蚀前沿、边界线、切口

**顺风侧 / 迎风侧**:
顺风侧是风吹向的一侧，也是侵蚀前沿最后到达的一侧；迎风侧是消散开始的一侧。
_Avoid_: 把"顺风"理解成风来的方向

**最后消失的那一片**:
分界线走到最后才吃掉的那片材料；它是一块沿扫描方向拉长、落在顺风侧的不规则区域，不是卡片中心的一个圆。
_Avoid_: 留存带、慢车道、留存圆、中心残留（用户 2026-09-02 明确要求不用这类说法）

**羽流**:
已脱离本体的粒子在空中形成的连续云团，有亮面、暗面和二者之间的折脊。
_Avoid_: 粒子群边缘、尾巴

## Relationships

- 一次 Dialog 粒子消散有且只有一张**释放场**；**分界线**是它在某一刻的等值线
- **分界线**从迎风侧推向顺风侧，其上有一段走得慢，走得慢的那一段最后剩下**最后消失的那一片**
- 每一格材料在到达自己的释放时刻时脱离本体、进入**羽流**；控件本体自身不移动、不变形
