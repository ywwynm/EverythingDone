...
软件编码与硬件编码的耗时相差一到两个数量级。

- 自动档只使用硬件编码器；软件实现仅在用户显式选中该编码器族时启用。
- 同一档内硬件候选一律排在软件候选之前，该顺序优先于编码模式匹配。
- 软件实现在能力诊断、设置摘要与导出完成信息中明确标注。
- 「自动」HDR 格式因此要求存在硬件 HDR 通路；仅软件可编的格式仍可显式选择，选中时编码器
  自动迁移到唯一能够编码该格式的族。
...
120 fps 通过）。HDR 因此只能由软件 AV1 承担，且仅到 60 fps；按 D53，自动档不使用软件编码器，
故该机自动档落到 SDR，用户显式选择 HDR10 或 HLG 时才会启用软件 AV1。
