...
## 2026-06-30 - 私密记事移动也鉴权（对齐私密文件夹），推翻"移动不看记事自身"

起因：用户反馈——把私密文件夹移到另一文件夹（拖拽 / "移动到文件夹"对话框）会鉴权，但把私密记事这样移动却不鉴权，不对称。

**推翻 2026-06-29"会话级访问即信任扩展到记事"决策的第 4 条**（"移动只看源 / 目标文件夹私密、不看记事自身，移动不暴露内容，维持不动"）。该条理由"移动不暴露内容"对私密文件夹同样成立，却唯独文件夹移动鉴权（`needsFolderMovePrivacyAuthentication` 看被移动文件夹**自身**的 `isFolderEffectivelyPrivate`），记事移动只看源 / 目标文件夹、漏了 `thing.isPrivate()`，本身就不自洽；且记事自身私密的会话鉴权体系（A1/A2、打开详情 / 右滑 / 调外观）此后已建立，移动成为唯一漏接"记事自身私密"的鉴权点。

决定：

1. **记事移动鉴权口径与右滑 / 文件夹移动完全对称**：`记事自身私密(thing.isPrivate()) ∨ 源文件夹有效私密`，且 `记事自身与其所在文件夹本会话均未认证` 才要求验证；目标为"未认证的有效私密文件夹"也要求验证。于是"自身私密、却处于非私密文件夹"的记事移走时会鉴权，与私密文件夹移动一致。
2. **访问即信任**：移入 / 移出已认证的私密文件夹、或已单独认证过的私密记事免重复验证（沿用既有 `!isFolderPrivacyAuthenticated` / `!isThingPrivacyAuthenticated` 豁免）。
3. **鉴权通过即 mark 本次移动的私密记事**（`markThingPrivacyAuthenticated`），与文件夹移动 mark 源 / 目标文件夹对称——移动验过身后，本会话再点开 / 右滑 / 再移动同一记事免二次验证、其卡片按 A2 揭示。否则会制造"移动验过、点开又要验"的新不一致。
4. 三个入口（拖拽、"移动到文件夹"纯记事对话框、混合对话框）统一生效；批量移动逐条复用单条口径，不再各算。

## 2026-06-30 - 取消私密批量补"已认证豁免"；设私密维持不鉴权（大厂调研佐证）

承上轮"私密管理操作鉴权口径"复盘，定两点：

1. **取消私密：批量 / 单选 / 混选补已认证豁免。** `toggleSelectedPrivateBatch` 取消分支旧实现无条件弹验证，与"访问即信任"不一致（文件夹 overflow 取消经 `shouldProtectFolderForAccess` 早已豁免，长按列表批量却漏了）。改为复用状态变更同款判定 `needsSelectedStateChangePrivacyAuthentication`：选中项本会话全部已认证则免验直接取消，仅当含"未认证的有效私密"项才弹验证。取消后项目由私密变公开，无需 mark（apply 取消分支本就 `clearThingPrivacyAuthenticated`）。属对 2026-06-28"访问即信任"决策第 3 条（批量取消保留验证）的细化——其理由"可能含未认证成员"在全部已认证时不成立。

2. **设私密：维持不鉴权（仅要求已配密码），不推翻 2026-06-28 决策。** 大厂调研佐证：把已有内容"设为私密"属"标记式隐藏"（打标记、不移位置），对照 iOS 照片隐藏——隐藏动作不鉴权、仅访问"已隐藏"相簿才验证；iOS 备忘录锁定语义更重（逐条加密）也不在锁定动作设卡。真正要鉴权的是"移入受保护容器"（对照 Google 锁定文件夹 / 三星安全文件夹 = 本应用的移入私密文件夹，已鉴权）与"访问 / 取消 / 移动私密项"（对象已是秘密）。设私密的对象在操作那一刻仍公开，鉴权挡不住泄露、只增整理摩擦，故不加。
