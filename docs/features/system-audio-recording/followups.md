...
## 已完成：系统授权返回后的方向样本延迟只提示一次（2026-08-17）

MediaProjection 授权成功返回后开始等待方向样本；1.5 秒内到达任一样本即静默取消，否则
弹出长 Toast，说明倾斜响应可能暂时不可用并建议修改“设备动作与方向”权限。提示状态持久
记忆，安装期间最多出现一次；取消授权、普通页面往返、关闭实时倾斜及正常快速恢复均不触发。
文案不包含任何系统品牌。状态门禁定向测试和全量单测通过；真机确认第一次受限返回提示、
第二次不重复提示。详见 `analysis-2026-08-17-fablesol-tilt-after-source-switch.md`。

另有一个与本次现象无关的独立潜在缺陷：transient `EGL_BAD_SURFACE` 分支会撤掉帧回调，
但本次所有复现均无 EGL failure，故不得作为本问题的修复目标；如后续出现实际 swap failure
日志，再单独立项处理。
