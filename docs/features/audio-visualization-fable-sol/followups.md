...
- **`swell_max_dp` 是无人消费的死参数（2026-07-22，D178）**：
  参数表里定义为「涨落上限」`124/122/120/118/119/120/120/118/117`，但
  `FableSolParams.kt:94` 与 `params.py:154` 之外**两端都没有任何代码读它**，
  decisions/followups 也查不到移除记录，疑为重构时静默遗弃。D178 选择改
  `water_level_goal_dp` 的映射曲线而非接回钳位，因此它仍然悬空。下次碰水位相关
  代码时决定：要么接回作为硬上限，要么连同调参 UI 一起删掉。**不要两者都不做**
  ——留着一个不生效的调参项，会让下一个人以为拖动它有用。

- **「已生成的浪完全不再变化」尚未实现（2026-07-22，D178）**：
  用户原始诉求是「浪从形成到移动到消失保持不变」。D178 只修了冠部 mask 覆盖不全
  这处不一致，没有全局关闭 ambient / 方向场 / 波包 / 轨道——它们是水面细碎质感的
  全部来源，全关会让水面退化成纯波方程的光滑曲面。若目测后用户仍要，做成开关而
  不是直接删。
