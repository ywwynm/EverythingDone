...
- **录音对话框的 PREPARED / STOPPED 空转态可用完全冻结原语（2026-07-30）**：这两态没有
  任何音频输入、水面 alpha 只有 0.16，却照样每帧 build+draw、面板照样被投票在 120Hz。
  STOPPED 尤其能拖——用户在那儿改文件名，想待多久待多久。fablesol-video-export D187 已经
  做出了完全冻结原语（`WaveVisualizerFableSolHost.setFrozen`：停帧循环 + 冻模拟 + 撤帧率
  投票 + surface 重建时按需单帧），录音对话框直接调即可。**但这是产品行为变更而不是纯
  优化**：空转态的水面在流动是视觉设计的一部分，冻住它需要先确认观感。备选是空转态降帧率
  （`FableSolFramePacer.setTargetFps`），保留流动感、省下约 3/4 的帧成本，代价是 GL 线程
  唤醒次数不变。
...
- **人眼眩光的 Android 移植（2026-07-24，等目测定型；D207 后收窄）**：移植面 =
  Kotlin 星光轨迹（gl_optics 的银边公式 CPU 复算 + 线性余辉 + 簇内主从，与闪点
  track 同族）+ 眩光小链（半分辨率星点注入/弥散/针芒 gather + 全分辨率 resolve）。
  D207 撤销了 water.frag 眩光源补丁，**共享 GLSL 无需任何改动**。闪点重新启用时
  若也要星光，把 glint track 的强度并进星光源列表即可（无需动 optical.frag）。
- **眩光可选细节（2026-07-24，低优先级）**：sinc² 周期节点（针芒串珠）当前未做；
  GUI 的 HDR 画布恒 hdr_requested=True，桌面 GUI 里看不到"SDR 态"眩光现场预览
  （离屏 glare_report 已覆盖双态）——若需要 GUI 预览可加录音态模拟开关。

## waterEdgeCoverage 的各向异性软带（2026-07-25，D219 附带发现）

`waterEdgeCoverage()` 仍用 `smoothstep(-0.5*fwidth, 0.5*fwidth, insideDistance)`。
换算到真实像素距离后软带宽度 = `fwidth/|∇| ∈ [1, √2]`，也就是斜坡上的层边界
