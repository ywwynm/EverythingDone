...
## 2026-09-13 修复粒子交接首帧的海浪网格缺行

真实附件→录音捕获到首次冻结帧有 4,704 个零位置顶点，下一活动帧即恢复，场景 framebuffer 中复现巨大三角形。上一轮将数组延迟初始化后，四个 `lazy(NONE)` 数组恰好在并行行任务中首次读取，各工作线程分配不同数组、丢失部分行。现在由 GL 所属线程先取齐顶点、投影和斜率数组，再并行写入。初次就被冻结时，以零步长合成基准水面并接收初始重力，不推进物理时间。六次首次网格与 36 张冻结／恢复帧未再出现零顶点，基准水位连续。消失时的 Surface 保护在共同粒子入口处理；详见[复现和验证](../dialog-particle-dismiss/repeated-appearance-and-wave-handoff-2026-09-13.md)。

## 2026-09-13 减少粒子弹窗交接期间的海浪初始化竞争

录音和音频窗口出现、消失时，海浪重型模拟、映射、星光、网格及直接缓冲改为在既有 GL 所有者线程首次使用时初始化，减少 UI 构造阻塞。合并冻结状态下重复首帧请求，跳过尺寸未变化的重复 resize；保留音频输入、暂停／导出冻结状态、GL 绘制及视觉模型。真实录音准备、音频播放及 Surface 快关回归通过。粒子侧完整时序与像素证据见 [呈现连续性诊断](../dialog-particle-dismiss/presentation-continuity-2026-09-13.md)。
...
## 2026-07-22（第十节）画外余量核算与巨浪轮廓的顶/坡解耦（D177）

起点是用户问「dialog 右侧是不是还有看不见的空间用于波浪生成，多大，代价多少」。核算
结论：物理网格 216 点覆盖 716.7dp，Android 实测容器 276dp，**画外占 61.5%**（每侧
220.3dp = 出生带 96 + 海绵区 80 + 尾段 44）。渲染窗口是另一回事——`requiredHalf` 算得
180.2dp、110 原始列（与 D32 记的实测「水平 110」精确吻合），比可见区宽的 42.2dp 里
58% 是弱透视必需、24% 是 Gerstner 轨道、16% 是 D31 的边界余量，**没有可省的冗余**。
物理侧真正的死区只有最外 12.3%（约 0.17ms/帧），不值得动 `N_POINTS`（会连带改 `DX_DP`
与 CFL，D139 明确禁止）。画外空间真正的代价在响应延迟：波群包浪峰入画 0.40s、装饰点击
0.22s、巨浪 1.08s。

由此转入巨浪轮廓（D177）。**中途出过一个需要记住的错误**：对比 Hann 与 quintic 时，
把 Hann 的「相对半宽」当成了「相对全宽」，得出「Hann 顶宽 41%、quintic 只有 24.7%」，
方向完全反了，并据此向用户建议过换窗。统一口径重算后是 Hann 0.2048W < quintic 0.2466W
——Hann 更尖。教训：跨函数比形状，第一步必须把所有量纲归到同一个分母上，尤其当两个
实现各自用「半宽归一化」和「全宽归一化」时。

最终改动见 D177：轮廓拆成平顶 + 侧翼，`P=0` 逐点退化回旧轮廓；出生/离场改用 3% 等高线
而非支撑边界。顶宽 193→327dp、入场 1.08→0.91s，侧翼陡峭度与振幅上限逐位不变。

Python 295 tests + Android 227 tests 全绿。**尚未在模拟器或真机上目测**，也未发布
阿里云——形状类改动的最终裁决只能靠眼睛。

## 2026-07-22（第十一节）真机反馈四修：DC 污染、冠部 mask 覆盖、流速惯性、水位软饱和（D178）

用户在 `202607220358` 上一次报了四个问题。前两个同源——都指向「巨浪的影响没有被正确
归类」：它的隆起被当成了层的直流分量（导致整幅画面在触发/离场时瞬间平移），而它的
支配 mask 又没能覆盖方向场与轨道（导致平顶上浮着约 20dp 的残留起伏）。

**排查中最有价值的一步**是不相信第一版修复。改完「从均值里扣掉巨浪」后位移只从
29.4dp 降到 23.2dp，于是逐层往下测：`heights` 层面 bias 与均值变化残差为 0、untouched
列完全不变；`surface` 层面 world_eta 在 untouched 区变化 0.0000——两层都干净，问题必
在更下游。最后定位到 `gl_scene` 的 `means = mean(sim.heights, axis=1)`：**深度基准高度
是同一个被污染均值的第二个消费点**，而且是大头。教训：一个量有几个消费点，就要几处
一起改；只验证「我改的那处对了」不等于问题解决了。两端因此各收敛出一个统一取值口。

另外 bias 本身第一版也算漏了——只扣了巨浪自身隆起，没算冠部压制背景带来的均值变化。

流速与水位是两个独立改动。水位那条先纠正了一个前提：水位与七境档位无关，
`level_goal = 160 × water_drive01`，用户感到的「PEAK 水位高」实为响度高。用户裁定目标
216dp 且不走钳位，因此改成软饱和，低端斜率与改前一致、只压高端。

顺带发现 `swell_max_dp` 两端都无人消费，是被静默遗弃的参数，已记 followups。

Python 297 tests + Android 227 tests 全绿。已发布阿里云（见 debug-update-notes）。
**四项全部需要真机目测裁决**，尤其流速减速的 1.80s 半衰期与水位的新曲线。

## 2026-07-22（第十二节）流速范围核算与静息流速回归（D179）

用户先问流速范围，答：0 ~ 200dp/s（L0 满驱动），每层上限 = `flow_base_dps` + 20 的
可闻补偿（200/187/171/154/147/137/125/120/109）。顺带澄清「最低流速是 18.4 还是 20」
——**都不是**，低端连续趋零：`drive` 线性趋零、`trim_gate` 是 smoothstep 也趋零，
20dp/s 只是 trim 项在 K≥0.10 时的满值。

随后用户给了一段自称「非常静止的环境」的录音要求分析。**这一步的关键是先验证探针**
（memory 里的旧教训）：同一脚本跑 `rec_20260721_210358.wav` 得 flow01 中位 0.18、
L0 峰值 199.8dp/s（贴合理论上限 200），证明探针走的是产品路径；对用户那段输出的
全零才可信。

那段录音 RMS **−84.0 dBFS**，比历史 11 段（−30.7 ~ −52.8）低 31dB 以上，约 2 个 LSB。
判 IDLE 正确，但也因此完全没有水流。另外提醒了用户：flow=0 时画面并非冻住——方向场
与环境波按 `ω=√(gk)` 自由传播（最长模态 46.2dp/s），他看到的「非常非常慢的流动」
应当是这个而非水流。

用户由此提出恢复基础流速并问「什么时候改没的」。用 `git log -G`（`-S` 抓不到纯数值
改动）逐提交追出完整轨迹：0.24 → 0.18 → 0.0，7-20 的因果耦合重构有意归零。改回 0.10
后复验那段录音：L0 18.0dp/s、flow01 与水位仍恒 0。

Python 297 tests + Android 227 tests 全绿。**−84dBFS 这个电平本身仍未解释**，已记
followups——它低到不像正常手机录音底噪。

## 2026-07-22（第十三节）「两端不一致」的报告不成立；根因是录音链路（D180）

用户报告 Python 打开录音 wav 完全不触发巨浪、一直 GROOVE，Android 却在莫名其妙的时刻
触发两次。三层排查：

1. **profile 判定**：文件名 `20260722145549-lose-my-mind.wav` 因时间戳后加了曲名而
   不匹配 `^\d{14}$`，静默退回 master。用 master 跑同一文件，逐条复现用户描述
   （0 次巨浪、GROOVE 63%、climax_score 恒 0）。已修正则并加正反测试。
2. **两端一致性**：同为 capture profile，Python 19.4/59.4s、Android 19.4/58.9s，
   状态分布差 1~6 帧。**不存在两端不一致。**
3. **触发时刻**：母带 4/4 精确命中用户认可的时刻，问题只在录音版。

第三点用合成对照定位。**过程中犯了本轮最值得记的错误**：第一版合成脚本把立体声
`mean` 成单声道，`source_channels` 2→1 触发 D176 的上混补偿分支，恒等对照（母带原样
转 wav）从 4 次掉到 0 次——而我当时**没有先做恒等对照**，直接拿它去比"截断"和"前导
静音"，得出三个错误结论并已经写进了 decisions，只能撤回重写。保留立体声后恒等对照
通过，结论才反转：前导静音与截断都无罪，合成对照精确命中 54.9/71.4s，**唯一变量是
录音链路**。

另一个方法教训：对齐录音与母带时，包络互相关给出 83.87s、onset 互相关分段标准差
32~35s，全是错的（该曲 107~123s 有高度重复段落 + 外放频响差异）。最后靠段落边界与
结构事件反推出 +1.8s，与用户自己的估计一致。**结构性特征比互相关峰值可靠得多。**

同时把 `idle_flow_ratio` 从 0.10 调到 0.16（用户指定）。Python 297 + Android 227 全绿。
capture profile 的重新拟合见 followups，本轮未做。

## 2026-07-22（第十四节）Python GUI 改为有序实时分析批次（D182）

修复了产品 GUI 与离线验证器口径不一致的最后一段链路：音频进程现在发送带 generation
的完整 `analysis_batch`，`AudioClient` 丢弃 load/seek 前的旧代际数据，`MainWindow`
逐批逐 hop 消费并在批内交织事件。实时模式彻底停止注入离线 section；离线导演行为保持
不变。新增了进程批次完整性、客户端代际过滤、多 hop 消费、事件观测批次边界和实时/离线
隔离测试；定向 25 项与最终 Python 全量 306 项（另 26 个 subtests）通过，尚未进行人工
GUI 目测。

## 2026-07-22（第十五节）纯实时巨浪门控精确收敛与控制曲泛化（D181）

本轮先纠正验收工具，而不是继续按旧结果调阈值。旧 `rt_probe` 会把立体声压成单声道，
旧 GUI realtime 又会在离线 timeline 就绪后注入未来信息；两者都不能代表产品实时路径。
新回放保留 `source_channels`，以 512 sample feed 批次记录事件的实际观察时刻，60Hz 下逐批
逐 hop 消费。最终结果：

- 《Lose My Mind》母带 **4 次且只有 4 次**：53.664 / 69.493 / 139.424 / 157.664s；
- 手机录音覆盖段 **2 次且只有 2 次**：54.507 / 72.277s；
- 《银花玉鉴逐人来》：143.360s。

23～32s、58s、98s、121.7s、166s 与 175s 等候选都被作为误触逐项审计。最后没有采用歌曲
名、绝对时间或固定每段次数，而是区分普通 physical attack、低水位 novelty/dense bridge、
section-backed 后续短语与已经重新鉴权后的 resurgence。验证器同时新增基数约束：即使第
5 次落在某个目标窗口里，也仍然失败。

额外用当前 assets 中三首非 WAV 原曲做泛化检查：`Take Me Hand` 共 3 次，最小间隔
14.955s；`Back To December` 与《带我去找夜生活》均为 0 次，未出现放宽阈值后的密集
机械触发。`tools/rt_probe.py` 与无头截图音频驱动也已改成同一纯实时批次语义。

## 2026-07-22（第十六节）Android 实时批次与节拍舍入 parity 收口（D183）

Android 两条动画路径改为保留 analyzer feed 的观测批次，消除了渲染 drain 时把多批事件
按声学时间全局回插的问题；新增跨批延迟事件与同刻 frame/event 顺序回归。master profile
绕过采集预热，Foote 阈值、minor 事件和 `state_sensitivity` 的 analyzer 传递与 Python 对齐。

录音版最初仍漏掉 54.507s。用同一 onset 序列直接重放两端 BeatTracker 后，定位到 Python
ties-to-even 与 Kotlin half-up 的舍入差异：76.5 被 Android 取成 77，连续三次后造成短暂
72.94 BPM 假换挡。三个离散化点改用 `Math.rint` 后，BPM/置信度逐 hop 差异归零；最终
Android 产品探针读取 48kHz 单声道录音，共 12841 个 hop、12841 个观测批次，只在
54.507 / 72.277s 触发两次。最后又审计并统一了 7 个会进入巨浪证据链的 ring 容量：
1.2s harmonic/low motion、2s onset/centroid、10s motion/loud/centroid context；它们在
48kHz 下曾因 round/truncation 语义相差 1 hop。修后真实录音触发时间与次数不变。

Android 全量 JVM 测试 246 项通过，另有 1 项条件探针跳过；未使用 ADB。

已发布阿里云 Debug `202607221030`（versionCode 43 / 2.0.0），APK 21,029,948 bytes，
SHA-256 `680e8c118702d312789e70e64aa6580188765c6d7f1c7c830b78afc57497aae3`。
`latest.json` 的本地 APK 哈希一致，完整 `releaseNotes` 已确认包含实现与验证章节。

## 2026-07-22（第十七节）全素材高潮泛化与录音 display/raw 双轨（D184～D185）

用户纠正了第十五节把 `Back To December = 0` 当作安全结果的判断：`assets` 中每首音乐都
有足以触发巨浪的高潮，同时《Lose My Mind》必须仍然严格只有 4 次。本轮继续使用已经校正
过的纯实时产品回放，不引入歌曲名、文件名、绝对时间或固定次数分支。巨浪门新增三类通用
证据：中高水位盲区的多证据到达；12 秒因果响度范围成熟后的滚动 q95 相对高潮；结构窗内
短而明确、独立确认的 downbeat。普通、相对高潮与结构短语的确认计时彼此隔离，14 秒可读
间隔保持不变。

11 条正式实音频轨道的 14 项基数与时窗约束全部通过。《Lose My Mind》母带严格 4 次：
53.664 / 69.600 / 139.424 / 157.664 秒；修改前的全录音严格 4 次：53.952 / 70.219 /
139.573 / 158.069 秒；短录音只覆盖前半段，严格 2 次：54.507 / 72.277 秒。《银花》母带
143.360 秒、全录音 143.776 秒，各 1 次。其余母带分别为 Take Me Hand 4、Back To
December 2、My Love 2、《带我去找夜生活》1、《再回首》1、《浮生未歇》1，全部至少 1 次。

旧 Android 全录音与母带的连续动画差异来自录音链，而非巨浪门。为避免录音补偿反向授权
巨浪，分析拆成 raw/display 双轨：raw frame、raw 七境和巨浪 gate 保持权威；固定的 display
响度展开及 low/mid/high `2.0 / 0.90 / 0.65` 份额校正只驱动水位、展示七境、颜色与普通
动画。补偿后 Lose 全录音的 level MAE 为 4.51dp、wave MAE 0.083、state 一致率 87.65%；
《银花》为 11.46dp / 0.218 / 66.09%。更强动态展开、kinetic 平滑和全局 band magnitude
都会恶化至少一条 Lose 对照，因而没有按单个房间/扬声器继续拟合；《银花》的非定常链路
残差留待多设备、多房间录音语料再校准。

Android 逐项同步同值门限与 display/raw 双轨后，两份全录音产品探针仍分别得到上述 4 次与
1 次巨浪。与 Python 同输入逐帧对比时，raw water、响度、噪声底、速度、motion、display
water 与巨浪累计次数一致；七境及 display evidence 只有 CSV 精度级浮点差异。Python
全量测试 333 项及 26 个 subtests 通过；Android 全量 JVM 测试 261 项中 260 项通过、0 失败、
1 项条件探针跳过。未使用 ADB。

最终发布阿里云 Debug `202607221306`（versionCode 43 / 2.0.0），APK 21,029,948 bytes，
SHA-256 `c7d6f00a497e1a994e2111f7bb62d28ce322c86dca697d847c6ac7323aa3e283`。首次发布后校验发现
功能日志含多个二级标题时，`currentDebugUpdateNotes(...)` 只截取第一段；将整篇调整为单个
二级条目、三级子标题后重新发布。最终远端 `latest.json`、APK `Content-Length`、本地哈希
完全一致，`releaseNotes` 已确认包含要求、门限、双轨及双端验证全部章节。

按用户要求在发布后重新跑了两份修改前的全录音。Android 与 Python 对同一录音的产品回放
没有明显差异：raw/display water 逐帧一致，Lose 与《银花》的 visible state 各只差 1 个
渲染帧，wave scale MAE 分别为 0.000578 / 0.000751；巨浪分别严格保持 4 次和 1 次。

录音相对母带方面，Lose 的 level MAE 4.506dp、wave 0.0833、state 87.65%、hero L0
1.931dp，已无明显差异；《银花》的频谱/颜色已接近，但 level 11.456dp、wave 0.2182、
state 66.09%、hero L0 8.583dp，仍有明显残差。随后继续消融固定 blend、固定非线性设备
曲线和严格因果 p95 inverse-AGC：前两者明显伤害 Lose；最安全的 p95 候选也只把《银花》
...
## 2026-07-24（第四十二节）波顶人眼眩光星芒（D206）

分析银丝"顶部更亮但无星光感"的根因：apex 机制已把超白收缩成每层 1~2 个点状
事件，但超出 headroom 的能量被 min() 丢弃、亮度严格停留在 2dp 宽的几何足迹里
——缺的是成像系统 PSF 的跨像素铺展。与用户讨论后按三项裁决（人眼形态 / SDR
也要 / 频率克制）在 Python 模拟器落地：`glare_pass.py` 桌面 PSF pass +
`python_optics_shader.py` 眩光源补丁 + 四个"光学机制试验"组参数
（glare_strength 0.9 / glare_threshold 3.2×白 / glare_needle_length 24dp /
glare_needle_count 6）。

迭代三轮实测修正：能量归一不可见 → 振幅制；逐通道减阈把粉白源削成纯红 →
亮度阈值保色相；/needleCount 把针尖压到阈下 + 16tap 梳齿串珠 → 去平均 +
32tap。验收 `tools/glare_report.py`：7% 帧有星、单帧 ≤2 簇、竖直针 ~20dp、
晕 13dp、SDR/录音双态 cap 合同成立；pytest 354 项 + 26 subtests 全绿
（新增 test_glare_pass.py 9 项，参数审计同步 12 滑杆）。改动未提交，留在
两仓库工作区。

追记（同日，D207）：用户目检指出"连排等大星"与"停留过短（完全不起眼）"。
星光源重构为 CPU 星光轨迹（银边公式 CPU 复算 + 确定性线性余辉 + 簇内四次方
主从 + 出生门），water.frag 眩光源补丁整体撤销（SDR 场景回到共享基线）。
新参数 glare_dwell_s=0.54s。实测：episode 停留 67ms→867ms（p50）、同帧星数
均值 2.09→1.05、SDR/录音 cap 合同不变；pytest 357+26 全绿。改动仍留工作区。

追记 2（同日，D208）：星与银丝合一改造——移除独立余辉（星振幅改为银丝辐亮度
的瞬时同步函数）、触发阈值按层权重放宽（L0~L6 全层出星、远层更弱）、apex 门
锁定波峰、新增前层轮廓遮挡；glare_dwell_s 滑杆移除（回到 12 滑杆），默认阈值
2.4→2.8。实测：episode 停留 1.3~1.9s = 银丝亮相时长，54.6% 帧有星、均值 1.9、
单帧 ≤5；pytest 360+26 全绿。

追记 3（同日，D209）：新增"眩光远层衰减"滑杆（CREST_RIM_WEIGHTS^k，默认
0.5，13 滑杆）；核查确认闪点逐层衰减表健在（被关的是 glint_capacity_gain
总门默认 0 与光晕权重全 0）。层振幅均值 L0 0.56→L6 0.14，测试全绿。

追记 4（同日，Android 移植 + 发布）：星芒按 D206~D209 设计移植到 Android——
FableSolStarField（CPU 星光轨迹，与 Python 逐式同构）+ FableSolGlarePass
（present 前 PSF pass，FP16 回退 RGBA8）+ 共享目录新增 glare_*.vert/frag
五个 shader（water/optical 七文件与其 hash 未动，Python 端 parity 不受影响）
+ FableSolParams/Tuning/调参 Dialog 五参数（13 语言字符串）。SDR 场景为
sRGB RGBA8 时 resolve 解码/回编码；EGL 重建走 invalidateGlObjects 惰性重建，
pass 失败回退原场景纹理。assembleDebug + fablesol 测试包（50 类 0 失败）
全绿；阿里云 debug 发布号 202607241136（versionCode 43），回填已写
memory/debug-update-notes.md。改动未提交。

追记 5（同日，D210 Python 先行）：按用户"梦幻模糊 + 旋转 + 长短不一"三项
裁决实现星芒梦幻化（宽晕二级模糊 + glare_halo 滑杆 + 泪膜慢旋/呼吸 +
星裙水色 0.62）；两轮修正（方晕→±3σ、针芒被宽晕淹没→呼吸抬底 + 针权重
0.78）。pytest 361+26 全绿；待用户目测确认后移植 Android 再发布。

追记 6（同日，D211 + 双端发布）：远层衰减默认 1.29、针长/线数上限 96dp/16
（默认 48/9）、新增芒长参差滑杆（幂指数 1.6）；D210 梦幻化（宽晕/慢旋/呼吸/
星裙水色）随本轮一并移植 Android。双端测试全绿；阿里云发布号 202607241241，
回填已写。改动未提交。

追记 7（同日，D212 + 发布）：芒长-强度耦合（needleTapWeight × lengthFactor，
参差 0~4）与宽晕格点修复（13 权重 ±12tap、σ/4 间距）双端落地；阿里云发布号
202607241304，回填已写。改动未提交。

追记 8（同日，D213 + 发布）：主刃偏水平（针表整体转 90°）+ 逐星图案差异
（种子派生朝向/转速/参差/针长，针芒重构为逐星全分辨率解析精灵，新
glare_halo.frag 合成 pass）双端落地；阿里云发布号 202607241325，回填已写。
改动未提交。

追记 9（同日，D214 性能 + 发布）：星系统每帧 1.52→0.86ms（九层合批/窗口
裁剪/零分配/预编译/tap 收敛），行为与视觉逐位不变；perf 探针脚本实测三轮
定位（切片无效→合批有效的根因：小数组 numpy 固定开销主导）。阿里云发布号
202607241357，回填已写。改动未提交。

追记 10（2026-07-25，D215 + 发布）：调参对话框按特效分组（水体透光/银丝/
星芒/闪点），"眩光"全面改名"星芒"（组名 + 7 标签 ×13 语言）；参数键与
已存值不变。阿里云发布号 202607241603，回填已写。改动未提交。

追记 11（2026-07-25，D216 + 发布）：body_light_strength 经分析判死（1.5%
像素可见性 + HDR 资格门 volume≤0.192<0.24 数学死路）后整项移除，双端参数/
构建分支/旧路径/13 语言字符串清理，共享 optical.frag mode 8 剖面保留以护
shader 哈希奇偶。阿里云发布号 202607250137，回填已写。改动未提交。

追记 12（2026-07-25，D219 + 发布）：银丝断续与上缘暗点定因修复。实机截图反馈
后先在 Python 端建三个探针（`scratch/rim_phase_probe.py` 相位相关、
`rim_compare.py` 双 shader 目录 A/B、`rim_rectified.py` 轮廓拉直展开），用
|亚像素相位|–逐列峰值相关系数 −0.548 定死根因为"亮芯落在 coverage×MSAA 双重
过渡区"，并用单变量消融排除波背暗带/闪点/星芒、证明轮廓上方无独立暗带。修复
只动共享 `water.frag` 的 `crestRimProfile()`（梯度模长距离 + 亮芯内移封顶 +
外侧对称衰减 + 像素盒卷积能量守恒），uniform/顶点格式/默认值全不变，两端因共享
assets 自动同源。相关系数降到 −0.154/−0.029，逐列峰值离散 5.4×→3.5×，最亮处
仅 +12%。阿里云发布号见下方回填。改动未提交。

追记 13（2026-07-25，D220 + 发布）：银丝"小矩形拼线"定因修复。新增逐像素粗糙度
指标（沿线亮度一阶差分 RMS / 序列 RMS）后，先用 FFT 排除高频噪声（≤6px 周期仅占
0~1%），再用"coverage 恒 1"与"解析近似梯度"两个消融把成因拆成屏幕导数的三角形
量化（约 40%）与有效 σ 不足一个采样间隔（主因）。修复：顶点新增分量 8 携带解析
|∇depth01|（两端 8→9 float，Python `_depth_gradient_magnitude` 与 Android
`writeDepthGradient` 差分规则一比一），σ 取 1px 下限并保持 width/σ 能量归一。
roughness 0.172→0.055，相位相关 −0.548→+0.042。代价是细档 FWHM 1.25→2.25px、
峰值 −14%，已在发布日志里向用户说明并给出"σ 下限降到 0.72"作为反向旋钮。测试同步：
Python 两处硬编码分量数改常量，Android `FableSolGlMeshLayoutTest` 断言改 9。
阿里云发布号 202607250428。改动未提交。

追记 13（2026-07-25，D221 + 发布）：用户在 D220 版上继续反馈"小矩形"，并猜测粗细
变化与 Z 轴翻滚遮挡——两者都成立，D220 的归因作废。定因过程：先用逐列 FWHM/峰位
量化证实粗细在 1.5~9.1px 间跳；再用"无丝列 vs 被更近层遮挡列"的重合率（L2 108/108、
L3 218/218、L8 146/146 = 100%）证实遮挡；最后发现 Android 的 `writeDepthGradient`
在层带倒转处把行列式钳到 1e-9、梯度爆到 1e7，使银丝铺满整列成亮块——这正是 Python
复现不出用户截图的原因（Python 侧本轮已加兜底钳制）。修复把顶点分量 8 从
...
## 2026-07-27 第 0 层清澈水体：调研、定案与 Python 实现

用户要求给第 0 层水体加一个"偏向透明"的设置项，使录音对话框与音频附件对话框呈现
"容器中的清水"；近水面保留记事色并有漂亮渐变，其它八层保持不透明，**且透明后不得
看到其它层出现在下方**。要求先调研物理规律、充分讨论达成一致再动手，Python 先行。

### 讨论中定下的六件事（详见 D225）

1. 透出来的是 `uPreWaterScene`（只画环境的那张 FBO）——它就是这个对话框的底色本身，
   天然不可能露出其它层。`uLayerAlpha[0]` 那条路作废（它混的正是 1~8 层的合成）。
   SurfaceView 真透明也作废（打洞后卡片背景不在其下，真透出的是被 dim 压暗的后台）。
2. 物理机制取"掠射 Fresnel + 斜程 Beer"，但实测发现单独不够（水线只到 0.327），
   补上第三项**近表背向散射纱**——它的尺度与视角几何无关，是唯一能在既定 36~38°
   相机下把水线压到不透明的机制。
3. 焦散一并做，一次到位。
4. 设置项形态：调参 Dialog 滑杆。
5. 导出跟随（导出卡片内走同一个 themedContext，底色与屏上完全一致）。
6. 不做容器壁，保持圆角硬裁——与"浪以完整形态流出画面"的既有世界观一致。

### Python 落地

- 新增 `ui/clear_water_policy.py`：视角几何、Fresnel、散射纱、逐通道 σ、焦散压缩比的
  共享推导，Android 侧须逐条镜像。
- `core/prototype_surface.py` 新增 `sample_caustic_hessian(x, z)`：方向谱与波包的相位是
  `kx·x + kz·z` 的解析式，可直接延拓到 `z<0`（观察者脚下那片水面）；宏观九层结构的
  Catmull 表不外推。顺带把振幅计算提取成两个私有方法，与主渲染共用。
- `ui/gl_scene.py`：`_caustic_field` 生成 24×128 的压缩比场（均值归一），
  `_macro_curvature` 补上第 0 层宏观行的 x 曲率。
- 共享 shader `water.vert`/`water.frag`：唯一的合成点是 front fill 分支里的一行
  `mix(既有输出, 透过水看到的背景, (1−R)·(1−veil)·T) * causticModulation()`。
- 三条新滑杆：水体清澈度、近表散射层厚度、焦散会聚深度（+ 焦散强度）。
- 新增 `tools/clear_water_report.py`：按清澈度 × 卡片明暗出对照图并打印权重曲线、
  焦散统计与画面变化量。

### 实现期修正（详见 D226）

σ 的定标基准从法向改为斜程；焦散旋钮从对比增益改为**会聚深度**（真实容器只有 96dp、
焦平面在 820dp，物理值只有 ±6% 的对称起伏）；焦散改为调制下行辐照度而非只调制背景。

### 状态

Python 完成待用户定档。`clear_water_gain=0` 对 `git show HEAD` 的七个 shader 做真实
A/B 渲染逐位一致，全量 363 条测试全绿。Android 移植与实机性能未开始。

### 补记：参数注册了但面板是白名单

用户反馈"Python 端完全没看到清澈水体这个设置组"。参数、shader 与渲染链全部就绪，
但 `ControlPanel.__init__` 的分组是显式白名单（逐个 `_auto_group(...)` 注册），漏注册
的组整组不显示。补上 `_auto_group(CLEAR_WATER_GROUP, "仅 GL 后端")` 后四条滑杆正常。
