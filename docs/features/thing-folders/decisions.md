...
2026-07-05 修订：上述顶部栏背景必须作为一个整体绘制，不能让 `view_status_bar` 与 `actionbar` 分别应用同一背景。纯色下差异不明显，但渐变背景会在两段各自重启，导致 statusbar/actionbar 明显断开。实现上应把二者放入同一个顶部 chrome 容器，容器绘制根目录 `accent+accent2` 或当前文件夹背景，子 View 保持透明。
...
## 2026-06-27 - 搜索只匹配用户可见的记事文本

首页搜索匹配记事时，只应使用用户可见的标题和正文。私密记事标题前缀、checklist 的存储编码前缀、状态位、层级位和控制标记都不属于真正内容，不参与搜索匹配。实现上由 `ThingSearchHelper` 统一生成搜索可见文本：标题只移除真实私密前缀，checklist 正文复用 `CheckListHelper.toContentStr(..., "", "")` 转成纯文本。

搜索入口、文件夹递归计数、文件夹缩略图候选、搜索范围内操作收集、编辑返回后的“是否仍匹配搜索”都应走同一套 helper，避免 DAO raw SQL 与内存过滤规则分叉。DAO 可以继续用 SQL 做状态、类型、文件夹等结构性筛选，但 keyword 的最终判断必须在净化后的用户可见文本上完成。
