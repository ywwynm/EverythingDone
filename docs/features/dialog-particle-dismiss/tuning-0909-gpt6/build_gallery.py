...
<header><div class="eyebrow">EVERYTHINGDONE / 0909 / R33</div><h1>粒子消散 · 卷束与微片质感</h1><p>对比 r31／r33：改善灭霸、科比的弯曲粒束，并同步复核自有弹窗的白底过渡、微片明暗和任意方向。</p></header>
...
const kinds={'compare-versions':'版本对比','compare-phase':'华为对照 · 进度对齐','compare-file':'华为对照 · 文件时间','compare-source':'真机截图对照','animation':'独立动画','eight-directions':'八方向同屏'};
const order=['ironman','thanos','kobe','language','color','attachment','attachment-image'];
const kindOrder=['compare-versions','compare-phase','compare-source','animation','eight-directions','compare-file'];
...
function select(v,play=false){current=v;video.src=v.file+'?v='+manifest.version;video.poster=v.poster+'?v='+manifest.version;video.playbackRate=1;$('title').textContent=`${v.title}｜${kinds[v.kind]}`;$('badge').textContent=`${v.rate} 倍 · 60 帧/秒`;$('download').href=v.file;history.replaceState(null,'','#'+v.file);const notes={'compare-versions':'按相同进度比较：上一版 r31、本轮 r33。照片序列另有华为参考列（映射为 1 秒）；自有弹窗为新旧两栏。','compare-phase':'左右按相同进度比较。华为参考区间重新映射到 1 秒；这不是参考录屏的文件原速。','compare-file':'左侧保留参考文件时间，右侧按 1 秒模型时长播放后停留。录屏可能已经慢放，不能据此推断实机动画时长。','compare-source':'左侧为静态真机截图，右侧为本轮动画。此场景没有同内容的华为原片。','animation':'完整前景和对应背景。未释放的内容保持原位，随后逐渐成为独立颗粒。','eight-directions':'同一画面内同时播放八个方向；模型本身支持连续任意角度。可在桌面预览程序中点击触点测试。'};$('note').textContent=notes[v.kind]+(v.scene==='thanos'?' 灭霸原片在转黑前截止，最后仍有轻微粒子。':'');renderList();if(play)video.play().catch(()=>{});}
...
select(files.find(v=>v.file===decodeURIComponent(location.hash.slice(1)))||files.find(v=>v.file==='attachment-compare-versions-0.5x.mp4')||files[0]);
</script></body></html>'''
