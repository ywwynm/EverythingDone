"""在去除尘缕的候选上小幅细化共同释放布局，不更换既有卷曲输运场。"""
...
def main():
    config=json.loads((OUT/'optimized-config.json').read_text('utf-8'));initial=np.array(config['release_grid'])
    ...
    def evaluate(values):
        c=copy.deepcopy(config);c['release_grid']=values.tolist();candidate(c,'combined');r=renderer.Renderer('ironman',ctx=ctx)
        ...
        r.close();loss=float(np.mean(errors)+.12*np.mean(contrasts)+.4*max(energy/reference_energy-1.4,0))
    ...
    for step in [.024,.012]:
        for index in range(25):
            ...
            for sign in [-1,1]:
                test=values.copy();test[index]=np.clip(test[index]+sign*step,initial[index]-.048,initial[index]+.048)
                loss=evaluate(test)
                if loss<lowest-.0005:lowest=loss;selected=test
            if lowest<best:
                values=selected;best=lowest;c=copy.deepcopy(config);c['release_grid']=values.tolist()
                (OUT/'refined-config.json').write_text(json.dumps(c,indent=2),'utf-8')
                print('改善',len(history),index,round(best,4),'耗时',round(time.monotonic()-started),flush=True)
            if index%5==0:(OUT/'shape-search.json').write_text(json.dumps(history,indent=2),'utf-8')
    (OUT/'shape-search.json').write_text(json.dumps(history,indent=2),'utf-8');ctx.release();print('完成',best,flush=True)
