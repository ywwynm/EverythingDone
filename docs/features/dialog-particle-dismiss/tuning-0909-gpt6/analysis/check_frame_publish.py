...
p=argparse.ArgumentParser();p.add_argument('serial',choices=['9018f404','R5CW20BLNKL']);p.add_argument('--output',default='device-frame-difference');args=p.parse_args()
out=HERE/args.output/args.serial;out.mkdir(parents=True,exist_ok=True)
def adb(*params):
    return subprocess.check_output(
        ['E:/AndroidSDK/platform-tools/adb.exe','-s',args.serial,*params],
        timeout=60 if params[0]=='install' else 20,
...
before=set(complete());adb('shell','input','keyevent','4');started=time.monotonic()
while time.monotonic()-started<8:
    lines=[line for line in complete() if line not in before]
    if lines:break
    time.sleep(.12)
