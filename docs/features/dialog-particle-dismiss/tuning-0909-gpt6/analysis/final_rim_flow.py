...
def main():
    ...
    assert qa['code_hash']==model['code_hash']==code and (qa['count'],qa['family_count'])==(128,30)
    assert len(model['model'])==18 and len(model['direction_checks'])==30
    family=read(HERE/'videos/family-videos.json')
    assert family['code_hash']==code and len(family['videos'])==30
    assert family['exporter_hash']==hashlib.sha256((HERE/'analysis/export_flow_family.py').read_bytes()).hexdigest()
    assert family['focus_exporter_hash']==hashlib.sha256((HERE/'analysis/rim_review_media.py').read_bytes()).hexdigest()
    ...
    assert maximum<1
    ui=read(HERE/'device-rim-flow/9018f404/touch-ui-checks.json')
    ...
    result=dict(model_hash=fingerprint,code_hash=code,videos=158,scenes=18,state_cases=234,distance_inputs=18,
        ...
        scope='加强解除边缘向外展开，按材料出生位置改变近远实际位移；参考时序与局部形态仍有差异，数值检查不替代审美验收。')
    if args.published:
