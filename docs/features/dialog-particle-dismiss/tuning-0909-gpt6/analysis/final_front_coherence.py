...
OUT=HERE/'analysis/front-coherence'
...
def main():
    ...
    for serial in SERIALS:
        data=read(HERE/'device-front-coherence'/serial/'touch-ui-checks.json')
    ...
    for serial in SERIALS:
        ...
        if a.published:
            published=read(HERE/'device-front-coherence'/serial/'published-checks.json')
    ...
    result=dict(model_hash=fingerprint,code_hash=code,videos=162,scenes=18,state_cases=234,distance_inputs=18,
        ...
        scope='实际渲染确认前沿衔接、角部收束、原纹理交接与拖尾宽度；两端采用共同规则，整帧误差不等于视觉相似度。')
    if a.published:
