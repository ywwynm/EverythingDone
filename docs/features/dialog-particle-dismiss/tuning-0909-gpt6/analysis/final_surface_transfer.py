...
def main():
    ...
    for serial in SERIALS:
        data=read(HERE/'device-surface-transfer'/serial/'touch-ui-checks.json')
    ...
    for serial in SERIALS:
        ...
        if a.published:
            published=read(HERE/'device-surface-transfer'/serial/'published-checks.json')
    ...
    result=dict(model_hash=fingerprint,code_hash=code,videos=162,scenes=18,state_cases=234,distance_inputs=18,
        ...
        scope='取消被否定的窄窗亮线；检查完整画面后细化覆盖、开放侧和渐隐。参考局部密度、形态及时序仍有差异，数值不代替审美判断。')
    if a.published:
