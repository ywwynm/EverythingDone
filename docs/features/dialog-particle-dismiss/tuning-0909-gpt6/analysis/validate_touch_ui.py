...
p=argparse.ArgumentParser();p.add_argument('serial',choices=['9018f404','R5CW20BLNKL']);p.add_argument('--output',default='device-flow-shaping');a=p.parse_args()
HERE=Path(__file__).resolve().parents[1];out=HERE/a.output/a.serial
adb=['E:/AndroidSDK/platform-tools/adb.exe','-s',a.serial]
def run(*args):return subprocess.run(adb+list(args),capture_output=True,check=True).stdout.decode('utf-8','replace')
def dump(name):
...
for label in ['near','far','back']:
    ...
    while time.monotonic()-start<8:
        lines=completions()
        if len(lines)>before:line=lines[-1];break
        time.sleep(.12)
